...
Scoped pass completed for historical `decisions/*`, `reference/*`, and `research/*` notes only. Applied canonical permalink replacements for clearly mappable legacy ADR-title and shorthand wikilinks in note content, including title-style ADR links and legacy shorthand aliases such as the git-sync ADR naming variants.

Concrete unresolved / intentionally deferred exceptions from this narrow slice:
- `reference/sync-improvements-implementation-plan` still contains `[[ADR-008 (desktop)]]`. This points to an external desktop ADR path named in source text (`desktop/.djinn/decisions/adr-008-task-sync-leveraging-git-based-sync-with-improvements.md`), not an in-project canonical memory note, so it should stay recorded as an external-reference exception rather than be remapped to an unrelated in-project ADR.
- Singleton/current-note aliases like `[[Roadmap]]`, `[[Project Brief]]`, and `[[V1 Requirements]]` remain outside this pass by design and are handled by separate narrow tasks.
- Non-ADR malformed or renamed references outside this slice (for example general design/reference exceptions not clearly proving a canonical target) should remain backlog exceptions unless a canonical permalink is directly verifiable.

This pass intentionally did not broaden into current-note routing fixes or singleton-alias cleanup.
