...
## Task Breakdown (Epic `52ok`) — All Complete

- Task `if6o`: ✅ Add ToolChoice enum and thread through LlmProvider::stream()
- Task `bw03`: ✅ Define finalize tool schemas and add finalize_tool_name() to AgentRole
- Task `z525`: ✅ Map ToolChoice to wire format in all provider format families
- Task `vizi`: ✅ Register finalize tools per role, remove generic tools from agent sessions
- Task `25gm`: ✅ Update role prompts to reference finalize tools
- Task `9m22`: ✅ Reply loop: finalize-tool detection, nudge loop, remove text-only break
- Task `95mc`: ✅ Finalize tool handlers and lifecycle processing
