...
> **Historical note on `agent_amend_prompt` (proposal z5f9 — 2026-07-09):** The migration
> notes below record that `agent_amend_prompt` was moved from Architect to Planner as part
> of ADR-051. That tool was **subsequently removed entirely** by proposal
> [[z5f9 — Remove the learned-prompt auto-coaching subsystem]]. `agent_amend_prompt` is
> **not part of the current Planner tool surface**. The migration history below is preserved
> for audit context; references to `agent_amend_prompt` describe a tool that no longer exists.
...
Completed in the same session as Epic B (2026-04-08), as a single atomic commit covering the backend rename, tool-surface migration, and the prompt content migration. Scope was larger than the ADR originally scoped because the backend rename couldn't ship without the tool-surface + finalize-hook + prompt changes or the first patrol after partial rename would hit a Planner session with no patrol prompt, no patrol tools, and no `patrol_schedule` logging path.

**Migration step 3 — coordinator patrol timer rename.** `maybe_dispatch_architect_patrol` → `maybe_dispatch_planner_patrol`; `DEFAULT_ARCHITECT_PATROL_INTERVAL` / `MIN_ARCHITECT_PATROL_MINUTES` / `MAX_ARCHITECT_PATROL_MINUTES` → `*_PLANNER_*`; `CoordinatorMessage::TriggerArchitectPatrol` → `TriggerPlannerPatrol`; `CoordinatorHandle::trigger_architect_patrol` → `trigger_planner_patrol`. Dispatch-side: `s.agent_type == "architect"` → `"planner"`; `resolve_dispatch_models_for_role("architect")` → `"planner"`; review task title `"Architect patrol: board health review"` → `"Planner patrol: board health review"`. The patrol-task title check (`task.title.contains("patrol")`) stays (role-agnostic). Test `patrol_skips_when_architect_already_running` → `patrol_skips_when_planner_already_running` with session fixture flipped to `agent_type="planner"`.

**Migration step 4 — tool surface migration.** `tool_schemas_planner` gained: `task_comment_add`, `memory_build_context`, `memory_health`, `memory_broken_links`, `memory_orphans`, `agent_metrics` (role_metrics), `agent_create` (role_create), ~~`agent_amend_prompt` (role_amend_prompt)~~. `memory_write` and `memory_edit` were already there and are preserved. `tool_schemas_architect` lost ~~`agent_amend_prompt`~~ (moved to Planner — agent-effectiveness review is a patrol action, not a consultant action). `code_graph` remains Architect-only. `submit_grooming` tool schema gained `next_patrol_minutes: Option<u32>` with a 5–60 range; `submit_work` lost its `next_patrol_minutes` field entirely (architect no longer self-schedules). All extension-layer tool_names + tool_schemas snapshots and the prompt-section snapshots were regenerated and inspected before accepting.
> **z5f9 note:** `agent_amend_prompt` was removed from the Planner tool surface by proposal z5f9 (2026-07-09). The tool no longer exists in the current architecture; strikethrough above marks the historical reference.

**Migration step 4 — finalize-hook migration.** The `patrol_schedule` activity logging moved from `roles/architect.rs::on_complete` to `roles/planner.rs::on_complete`. The Planner version is gated on `task.issue_type == "review" && task.title.contains("patrol")` so decomposition and intervention dispatches never log a patrol schedule. The coordinator reads this activity via the existing query in `coordinator/dispatch.rs` — no change needed on the read side. Architect's `on_complete` was simplified to just return `SubmitForMerge`.

**Migration step 4 — planner.md rewrite.** The new planner prompt has three workflows gated by a mode-detection table at the top:
- **Workflow A — Board Health Patrol** (new, migrated from architect.md): board overview, epic health check, approach viability, stuck work detection, memory health review, contradiction review, agent effectiveness review, corrective actions (all five cases: stuck task, task running that shouldn't, missing blockers, duplicates, epic-done-but-still-open), and self-scheduling via `submit_grooming(next_patrol_minutes=N)`. Reshape close sites explicitly set `close_reason="reshape" | "superseded" | "duplicate"` to cooperate with Epic B's reentrance guard.
- **Workflow B — Wave Decomposition** (unchanged from the old planner.md modulo a section rename).
- **Workflow C — Lead Escalation** (new, a short targeted workflow for `request_planner`-originated review tasks). Emphasizes dedupe / reshape / decompose / spike-dispatch-to-Architect.

**Migration step 6 — architect.md rewrite.** Stripped to: identity + the two contracts (unchanged content) + the `code_graph` sweep (§7 unchanged) + strategic ADR gaps (§8) + spike findings memory writes (§9, moved ahead of the escalation section) + a new **external reference hunt via `github_search`** section that teaches the consultant to reach for real-world code patterns via grep.app before deep-reading our own code. Bonus: the chat prompt gained a parallel `github_search` guidance paragraph to preserve the ADR-050 §2 parity contract.

Removed from architect.md: the entire patrol workflow, all five corrective-action cases, the agent effectiveness review, and the self-scheduling `next_patrol_minutes` section (architect no longer self-schedules). Architect no longer has a "Patrol Workflow" section at all — the top-of-file mission now states on-demand consultant only.

**Tests updated**: `architect_prompt_contains_memory_health_review` and `architect_prompt_contains_contradiction_review` → renamed and flipped to assert the content lives on the Planner prompt; new `architect_prompt_requires_spike_note_traceability` keeps the task-traceability assertions that still belong to architect. `tool_schemas_include_role_specific_tools` updated to expect `task_comment_add`, `memory_health`, ~~`agent_amend_prompt`~~, etc. on planner and to explicitly forbid ~~`agent_amend_prompt`~~ on architect. `tools_section_injected_into_rendered_prompt` updated with the inverse assertion on architect and positive assertion on planner.
> **z5f9 note:** These test assertions have since been converted to negative regression guards (asserting `agent_amend_prompt` is NOT present) by the runtime removal epics for z5f9.

Incidental: this commit also bundles a user-authored `github_search` HTTP-client fix (fresh `reqwest::Client` per call with a browser-style UA) — the shared `LazyLock` + `djinn-agent/0.1` UA was getting 429'd by grep.app even at low request rates. Scoped to `extension/handlers.rs::call_github_search` only.
...
Epic B and Epic A v2 both landed in the 2026-04-08 session. Remaining work:

1. **Epic C** (proposal pipeline backend). Largest scope; Epic B's active-session guard is the safety net Epic C depends on (already in place). Live-coordinator safety rule from the Epic C notes still applies — no live `epic_create` calls during implementation.
2. **Pulse panels** (frontend) come after Epic C. They consume the proposal lanes Epic C creates.
3. **Memory note update** (`project_canonical_graph_warming_architect_only.md`) is housekeeping that can ride along with Epic C or the Pulse work.

**Known loose ends from Epic B + Epic A v2 that the next session should track**:
- Close sites still emit only `"completed"` / `"force_closed"`; the new `CLOSE_REASON_{RESHAPE,SUPERSEDED,DUPLICATE}` constants exist and the Planner prompt now instructs patrol to set `close_reason="reshape" | "superseded" | "duplicate"` when force-closing during reshape. Worth verifying in practice that the planner actually emits these on the first few real patrol runs.
...
- **Epic A v2 nuance**: the Planner now has a unified tool surface across all three modes (patrol / decomposition / intervention) because Rust role configs can't gate tool availability per task. Mode-specific hygiene is prompt-enforced via the top-of-file mode table in `planner.md`. If the prompt-side enforcement proves unreliable (e.g. planner calls `role_amend_prompt` during a decomposition session), the next refinement is splitting `PlannerRole` into two role configs: `planner` for decomposition and a dedicated `planner-patrol` variant with the hygiene tools. Avoided for now because it doubles the dispatch surface.
- **First real patrol is the proving ground**. The next Planner dispatch on a `review` + "patrol" task will exercise the full migrated pipeline end-to-end for the first time. Worth watching the initial runs in Langfuse and checking for: (a) the planner correctly detects patrol mode, (b) it calls `submit_grooming(next_patrol_minutes=N)` at the end, (c) the coordinator reads the resulting `patrol_schedule` activity and updates `next_patrol_interval` on the next tick, (d) close_reason discipline is honored on any reshape force-closes.
