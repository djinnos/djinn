...
- [[decisions/adr-046-chat-driven-planning-drafting-epics-research-agent-deliverables-and-memory-write-access|ADR-046 Chat-Driven Planning — Drafting Epics, Research Agent Deliverables, and Memory Write Access]] — extended. Chat is the interactive form of Architect (per ADR-050); this ADR makes the symmetric consultant contract explicit.
...
- [[decisions/adr-047-repo-graph-query-seam-for-code-graph-tool|ADR-047 Repo-Graph Query Seam for code_graph tool]] — related. The bridge surface being fixed in Migration step 1 is the seam defined by this ADR.
- [[decisions/adr-043-repository-map-scip-powered-structural-context-for-agent-sessions|ADR-043 Repository Map — SCIP-Powered Structural Context for Agent Sessions]] — related. The canonical graph lifecycle is the infrastructure ADR-050 built on, which this ADR formalizes as "not an agent task."
