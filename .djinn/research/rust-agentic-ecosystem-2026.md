...
However: Djinn's agent orchestration is custom (Coordinator, Board, phases, task lifecycle). It doesn't deeply depend on a framework like Eino for its core logic. The MCP tool layer and HTTP server are the critical infrastructure pieces, and those are solid in Rust.
