...
**Decision: libSQL (native Rust)**

- First-class Rust SDK (libSQL is Rust)
- DiskANN vector search built-in for embedded mode
- Turso embedded replicas for desktop sync (local or over network)
...
Pure in-memory vector store with file persistence. Can run alongside SQLite/libSQL as a dedicated vector index without adding a second database process. Useful if DiskANN proves insufficient.
