...
`cargo check`/`cargo build` working without a running Postgres instance (e.g. on
CI or in clean clones), we use sqlx's **offline mode** backed by the `.sqlx/`
...
With the test Postgres running on :5433 (started by
`docker compose up -d postgres-test`), run from the repo root:
...
it is the offline schema cache that lets clean clones build without Postgres.
