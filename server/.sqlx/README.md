...
`cargo check`/`cargo build` working without a running PostgreSQL instance (e.g. on CI
...
With `DATABASE_URL` pointing at a migrated PostgreSQL database, or with the
Docker test PostgreSQL running on :5433, run from the repo root:
...
it is the offline schema cache that lets clean clones build without PostgreSQL.
