...
- **If a criterion references a specific diagnostic/inspection command** (e.g. `grep`/`rg`, `git log`, `git diff --stat`, `cargo tree`, `cargo metadata`, `jq` over a generated file, or just reading a generated file), **run it via `shell`** and check the output. You have shell access — use it for task-specific *inspection* that goes beyond reading the diff. Prefer commands that read what is already on disk: anything that has to **compile the crate first** (e.g. `cargo modules …`, `cargo expand`, `cargo doc`) is a build, not an inspection, and costs the session minutes it does not have.
- **Do NOT re-run the build/lint/test SUITES** (`cargo build`, `cargo check`, `cargo clippy`, `cargo test`/`nextest`, `go test`, `npm test`, etc.). Verification has already run those scoped commands and they passed before this review (that's what moved the task here) — re-running them just burns the session. If an acceptance criterion says "covered by tests", confirm the tests *exist and assert the behavior* by reading them, not by executing the suite.
- **The one exception:** if an acceptance criterion *names* a lint or test command as the evidence it requires, run exactly that command, scoped with `-p <crate>` (or the equivalent narrowest selector) — never workspace-wide, and never `--all-targets`/`--all-features` unless the criterion says so. `cargo check`/`cargo build` remain unavailable to you; use `cargo clippy -p <crate>`, which reuses the warm cache and lints at the same time.
