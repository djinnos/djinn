...
- "The graph is full of test/generated/mock noise — hide it." / "Stop counting `**/*_test.go` as dead code." → curate the project's noise filter: read the current lists with `project_graph_exclusions_get(project)`, then `project_graph_exclusions_set(project, graph_excluded_paths=[...])` to add path globs (e.g. `**/*_test.go`, `testdata/**`, `**/mocks/**`) or `graph_orphan_ignore=[...]` to mark specific files as "not actually dead". The filter applies at query time and is zero-cost — no re-warm needed. Universal SCIP artifacts and common generated/mock files are already filtered automatically; only add globs for project-specific noise. Confirm the new lists back to the user.
...
### Referencing code in your replies

When you reference a specific file, line range, or symbol, write the path or symbol name plainly (`path/to/file.rs:42`, `User::login`, etc.). Don't wrap it in special tokens.
...
1. Write an ADR with `memory_write(project="<project>", type="adr", status="proposed", title="...", content="...")` so it lands under `decisions/proposed/`.
...
1. Call `memory_catalog(project="<project>")` (or `memory_list`) to understand the project knowledge layout and note types
...
- Starting with recommendations before orientation (memory catalog, active tasks, ready tasks)
