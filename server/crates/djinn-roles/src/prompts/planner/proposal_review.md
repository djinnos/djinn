...
- **Out of the agent's model — human approval or organizational structure** — the criterion requires an approval workflow, sign-off gate, signed or delegated authority, separation of duties, approver/reviewer identity, CODEOWNERS mapping, or a named org role or deadline. Djinn writes code and opens pull requests; approval and merge are enforced by the forge and its configured owners. Drop the criterion, or rewrite it to the repository-checkable technical control it was standing in for, and record any required human approval as a runbook line instead of an AC.
