...
CREATE TABLE ci_route_attempts (
    ...
    -- when it was (wave 5).
    --
    -- Without this column, reporting cannot tell "Lead diagnosed" from "Lead
    -- never answered". The supervisor derives the fallback's
    -- `diagnostic_reason` from the route's own Tier-2 reason, so a Lead that
    -- timed out on a `causal_failure` route lands as
    -- `no_grounded_remedy` -- exactly the spelling a Lead that *did* answer
    -- and found no remedy produces. The two are opposite operational facts
    -- (one needs a better prompt or a longer deadline, the other needs a
    -- human) and used to be indistinguishable in the durable record because
    -- the rejection existed only inside a `tracing::warn!`.
    --
    -- NULL means the delivered result was accepted as submitted.
    lead_rejection          VARCHAR(48)  NULL,
    ...
    -- The closed rejection vocabulary. Same rule as every other durable
    -- vocabulary here: spelled once in Rust, once in this CHECK.
    CONSTRAINT ci_route_attempts_lead_rejection_check
    ...
    -- A rejection means the delivered result was replaced by the single
    -- diagnostic fallback. It cannot coexist with an accepted repair, park,
    -- or supersede -- those are results Lead actually produced.
    CONSTRAINT ci_route_attempts_rejection_pairing_check
...
-- The rollback quiescence report (proposal `nafu`, wave 5).
--
