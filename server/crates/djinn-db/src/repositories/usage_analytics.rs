...
mod tests {
    ...
    // ── Integration tests against real Postgres test DB ─────────────────────
    ...
    async fn query_totals_series_breakdown_no_alias_errors() {
        ...
        // Day 1 sessions
        let s1 = insert_session(&db, &proj, None, "model-a", "chat", "2025-03-10T10:00:00.000Z", 100, 50, Some(1.0)).await;
        let s2 = insert_session(&db, &proj, None, "model-a", "chat", "2025-03-10T11:00:00.000Z", 200, 100, Some(2.0)).await;
        // Day 2 session
        let s3 = insert_session(&db, &proj, None, "model-b", "worker", "2025-03-11T10:00:00.000Z", 300, 150, Some(3.0)).await;
        ...
        let result = repo.query(&params).await.expect("query should not fail with ColumnNotFound");
        ...
        let model_a = result.breakdown.iter().find(|r| r.group_key == "model-a").unwrap();
        assert_eq!(model_a.session_count, 2);
        assert_eq!(model_a.tokens_in, 300);
        assert_eq!(model_a.total_cost_usd, Some(3.0));
        let model_b = result.breakdown.iter().find(|r| r.group_key == "model-b").unwrap();
    ...
    async fn null_cost_semantics_preserve_none() {
        ...
        insert_session(&db, &proj, None, "model-x", "chat", "2025-03-10T10:00:00.000Z", 100, 50, None).await;
        // One session with priced cost
        insert_session(&db, &proj, None, "model-x", "chat", "2025-03-10T11:00:00.000Z", 200, 100, Some(2.0)).await;
        ...
        let row = result.breakdown.iter().find(|r| r.group_key == "model-x").unwrap();
    ...
    async fn model_effectiveness_shared_credit_no_inflation() {
        ...
        let task1 = insert_task(
            &db, &proj, Some(&epic), "closed", Some("completed"), 1, 0,
        ).await;
        insert_session(&db, &proj, Some(&task1), "model-a", "worker", "2025-03-10T10:00:00.000Z", 100, 50, Some(1.0)).await;
        insert_session(&db, &proj, Some(&task1), "model-a", "worker", "2025-03-10T11:00:00.000Z", 200, 100, Some(2.0)).await;

        // Another task touched by TWO models — shared-credit should count for both
        let task2 = insert_task(
            &db, &proj, Some(&epic), "closed", Some("completed"), 0, 0,
        ).await;
        insert_session(&db, &proj, Some(&task2), "model-a", "worker", "2025-03-10T12:00:00.000Z", 50, 25, Some(0.5)).await;
        insert_session(&db, &proj, Some(&task2), "model-b", "worker", "2025-03-10T13:00:00.000Z", 60, 30, Some(0.6)).await;

        // Failed closed task (no completed close_reason)
        let task3 = insert_task(
            &db, &proj, Some(&epic), "closed", Some("failed"), 2, 1,
        ).await;
        insert_session(&db, &proj, Some(&task3), "model-a", "worker", "2025-03-10T14:00:00.000Z", 10, 5, Some(0.1)).await;
        ...
        let (effectiveness, _matrix) = repo.query_effectiveness(&params).await.expect("effectiveness query should succeed");

        let model_a = effectiveness.iter().find(|e| e.model_id == "model-a").unwrap();
        let model_b = effectiveness.iter().find(|e| e.model_id == "model-b").unwrap();
    ...
    async fn user_attribution_via_coalesce() {
        ...
        let result = repo.query(&params).await.expect("query should succeed");
        let row = result.breakdown.iter().find(|r| r.group_key == user_id).unwrap();
    ...
    async fn proposal_grouping_through_epics() {
        ...
        insert_session(&db, &proj, Some(&task), "model-a", "worker", "2025-03-10T10:00:00.000Z", 100, 50, Some(1.0)).await;
        ...
        let result = repo.query(&params).await.expect("query should succeed");
        let row = result.breakdown.iter().find(|r| r.group_key == proposal).unwrap();
        assert_eq!(row.session_count, 1);
        assert_eq!(row.tokens_in, 100);
    }
    ...
    async fn project_model_matrix_includes_null_project_sessions() {
        ...
        insert_session(&db, &proj, None, "model-a", "chat", "2025-03-10T10:00:00.000Z", 100, 50, Some(1.0)).await;
        insert_session(&db, &proj, None, "model-a", "worker", "2025-03-10T11:00:00.000Z", 200, 100, Some(2.0)).await;
        insert_session(&db, &proj, None, "model-b", "chat", "2025-03-10T12:00:00.000Z", 300, 150, Some(3.0)).await;
        ...
        let (_effectiveness, matrix) = repo.query_effectiveness(&params).await.expect("matrix query should succeed");

        // Matrix should have project_id = proj for all rows since all sessions have a project
        let model_a = matrix.iter().find(|r| r.model_id == "model-a" && r.project_id == proj).unwrap();
        assert_eq!(model_a.sessions, 2);
        assert_eq!(model_a.spend_usd, Some(3.0));

        let model_b = matrix.iter().find(|r| r.model_id == "model-b" && r.project_id == proj).unwrap();
        assert_eq!(model_b.sessions, 1);
    ...
    async fn chat_sessions_without_task_included_in_totals() {
        ...
        insert_session(&db, &proj, None, "model-a", "chat", "2025-03-10T10:00:00.000Z", 50, 25, Some(0.5)).await;
        // Worker session with no task (edge case)
        insert_session(&db, &proj, None, "model-a", "worker", "2025-03-10T11:00:00.000Z", 100, 50, Some(1.0)).await;
        ...
        let chat_row = result.breakdown.iter().find(|r| r.group_key == "chat").unwrap();
        assert_eq!(chat_row.session_count, 1);
        let worker_row = result.breakdown.iter().find(|r| r.group_key == "worker").unwrap();
