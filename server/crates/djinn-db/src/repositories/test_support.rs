...
pub async fn seed_board_health_mismatch_candidate(db: &Database, project_id: &str, task_id: &str) {
    ...
    let creator_compact_id = creator_uuid.simple().to_string();
    let creator_login = format!("mismatch-test-{creator_compact_id}");
    let creator_github_id =
        i64::from_be_bytes(creator_uuid.as_bytes()[8..].try_into().unwrap()) & i64::MAX;
    sqlx::query("INSERT INTO users (id, github_id, github_login) VALUES ($1, $2, $3)")
        .bind(&creator_id)
        ...
        .execute(db.pool())
        .await
        .expect("failed to seed board-health mismatch candidate creator");
    sqlx::query(
        "INSERT INTO tasks \
