...
mod tests {
    ...
    /// Create a task in `closed` status with an explicit `closed_at`, returning
    /// its id. Used by completed-dependency-parent tests. Reuses the existing
    /// `create_task` helper (which creates a project + epic) and then flips the
    /// task to closed with a timestamp.
    async fn create_closed_task_at(db: &Database, task_id: &str, closed_at: &str) {
        // Uses untyped `sqlx::query` (not the compile-time-checked macro) so
        // test-only UPDATEs do not require `.sqlx` offline-cache entries.
    ...
    /// Wire a blocker edge: `blocking_task_id` blocks `task_id`. Uses untyped
    /// `sqlx::query` so the test INSERT does not need an offline-cache entry.
    async fn add_blocker_edge(db: &Database, task_id: &str, blocking_task_id: &str) {
    ...
    async fn completed_blocker_parent_summaries_orders_and_bounds() {
        ...
        create_closed_task_at(&db, &p1, "2025-01-01T00:00:00Z").await;
        create_closed_task_at(&db, &p2, "2025-03-01T00:00:00Z").await;
        create_closed_task_at(&db, &p3, "2025-02-01T00:00:00Z").await;
