...
fn assert_endpoint_closed(graph: &GraphResponse) {
    ...
    assert!(
        graph
            .edges
            .iter()
            .all(|edge| ids.contains(edge.source_id.as_str())
...
async fn lifecycle_graph_caps_after_selecting_nodes_and_closes_every_edge_layer() {
    ...
    let archived = repo
        .create(&project.id, "Archived Exact", "body", "reference", "[]")
        .await
        .unwrap();
    let deprecated = repo
        .create(&project.id, "Deprecated Null", "body", "reference", "[]")
        .await
        .unwrap();
    let active = repo
        .create(
        ...
        .unwrap();
    let tie_a = repo
        .create(&project.id, "Archived Tie A", "body", "reference", "[]")
        .await
        .unwrap();
    let tie_b = repo
        .create(&project.id, "Archived Tie B", "body", "reference", "[]")
        .await
        .unwrap();
    ...
    repo.upsert_typed_entity_association(
        ...
        MemoryEntityRef::note(&archived.id),
    ...
    assert!(
        default_graph
            .nodes
            .iter()
            .all(|node| node.entity_type == "proposal" || node.status == "active")
    );
    ...
    let zero = repo
        ...
        .await
        .unwrap();
    assert_eq!(zero.lifecycle_summary.as_ref().unwrap().inactive_total, 4);
    assert!(
        zero.nodes
            .iter()
            .all(|node| node.entity_type != "note" || node.status == "active")
    );
    ...
    assert_eq!(
        (
            summary.inactive_total,
            summary.inactive_returned,
            summary.inactive_omitted
        ),
        (4, 3, 1)
    );
    let inactive: Vec<_> = capped
        .nodes
        .iter()
        .filter(|node| node.entity_type == "note" && node.status != "active")
        .collect();
    assert_eq!(inactive[0].id, archived.id);
    assert_eq!(
        inactive[1].id,
        std::cmp::min(tie_a.id.clone(), tie_b.id.clone())
    );
    assert_eq!(
        inactive[2].id,
        std::cmp::max(tie_a.id.clone(), tie_b.id.clone())
    );
    assert!(
        capped
            .edges
            .iter()
            .any(|edge| edge.source_id == active.id && edge.target_id == archived.id)
    );
    assert!(
        capped
            .typed_edges
            .iter()
            .any(|edge| edge.kind == "supersedes")
    );
    assert!(
        capped
            .typed_edges
            .iter()
            .any(|edge| edge.kind == "derived_from" && edge.source_id == proposal.id)
    );
    assert!(
        !capped
            .typed_edges
            .iter()
            .any(|edge| edge.kind == "contradicts")
    );
