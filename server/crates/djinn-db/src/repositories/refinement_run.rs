...
impl ProposalRepository {
    ...
    pub async fn acknowledge_refinement_task_materialization(&self, request: AcknowledgeRefinementTaskMaterializationRequest) -> IntentMutationResult<bool> {
        valid_intent_request(&request.run_id, &request.intent_id, request.generation, &request.owner)?;
        ...
        if !correlated { return Err(RefinementIntentMutationError::TaskCorrelationMismatch { task_id: request.task_id }); }
        ...
        if changed { touch_heartbeat(&mut tx, &request.run_id, request.generation).await?; }
        ...
        tx.commit().await?;
        if changed || idempotent { Ok(changed) } else { Err(RefinementIntentMutationError::ClaimConflict { intent_id: request.intent_id, owner: request.owner }) }
    }

    /// Complete one phase and persist exactly one next-phase intent in the same transaction.
    pub async fn complete_refinement_intent(&self, request: CompleteRefinementIntentRequest) -> IntentMutationResult<RefinementNextIntent> {
        valid_intent_request(&request.run_id, &request.intent_id, request.generation, &request.owner)?;
        if request.next_round <= 0 || request.next_idempotency_key.trim().is_empty() { return Err(RefinementIntentMutationError::InvalidRequest("positive next round and idempotency key are required".into())); }
        self.db().ensure_initialized().await?;
        let mut tx = self.db().pool().begin().await?;
        ...
        if !completed {
            ...
            if !already { return Err(RefinementIntentMutationError::ClaimConflict { intent_id: request.intent_id, owner: request.owner }); }
        }
        let next_phase_name = phase_name(request.next_phase); let next_role_name = role_name(request.next_role);
        ...
        if completed { touch_heartbeat(&mut tx, &request.run_id, request.generation).await?; }
        tx.commit().await?;
        Ok(RefinementNextIntent { intent_id: row.get("id"), round: row.get("round"), phase: phase(row.get("phase")).map_err(snapshot_to_mutation)?, role: role(row.get("role")).map_err(snapshot_to_mutation)? })
    }
}

async fn touch_heartbeat(tx: &mut sqlx::Transaction<'_, sqlx::Postgres>, run_id: &str, generation: i32) -> IntentMutationResult<()> {
    ...
    if updated.rows_affected() == 1 { Ok(()) } else { Err(RefinementIntentMutationError::GenerationConflict { run_id: run_id.to_owned(), generation }) }
}
fn phase_name(value: RefinementPhase) -> &'static str { match value { RefinementPhase::AdversaryAttack => "adversary_attack", RefinementPhase::AdvocateRevision => "advocate_revision", RefinementPhase::JudgeAdjudication => "judge_adjudication" } }
fn role_name(value: RefinementRole) -> &'static str { match value { RefinementRole::Adversary => "adversary", RefinementRole::Advocate => "advocate", RefinementRole::Judge => "judge" } }

impl ProposalRepository {
    pub async fn park_refinement_run(&self, request: ParkRefinementRunRequest) -> IntentMutationResult<bool> {
        self.db().ensure_initialized().await?;
        let kind = match request.kind { RefinementParkKind::AwaitingReview => "awaiting_review", RefinementParkKind::AwaitingEvidence => "awaiting_evidence" };
    ...
    pub async fn terminal_refinement_run(&self, request: TerminalRefinementRunRequest) -> IntentMutationResult<bool> {
        self.db().ensure_initialized().await?;
        let context = serde_json::to_value(&request.reason).map_err(|e| RefinementIntentMutationError::InvalidRequest(e.to_string()))?.get("context").cloned().unwrap_or(serde_json::Value::Null);
    ...
    pub async fn record_refinement_durable_progress(&self, run_id: &str, generation: i32, _progress: RefinementDurableProgress) -> IntentMutationResult<()> {
        self.db().ensure_initialized().await?;
        let mut tx = self.db().pool().begin().await?; touch_heartbeat(&mut tx, run_id, generation).await?; tx.commit().await?; Ok(())
    }
    pub async fn load_refinement_lifecycle_aggregate(&self, proposal_id: &str, heartbeat_grace_millis: i64) -> IntentMutationResult<RefinementLifecycleAggregate> {
        if heartbeat_grace_millis < 0 { return Err(RefinementIntentMutationError::InvalidRequest("heartbeat grace must not be negative".into())); }
        self.db().ensure_initialized().await?;
        let stale = sqlx::query_scalar::<_, i64>("SELECT count(*) FROM refinement_runs WHERE proposal_id = $1 AND state IN ('running', 'parked') AND heartbeat_at::timestamptz + ($2 * interval '1 millisecond') < transaction_timestamp()").bind(proposal_id).bind(heartbeat_grace_millis).fetch_one(self.db().pool()).await?;
        let reaped = sqlx::query_scalar::<_, i64>("SELECT count(*) FROM proposal_revisions WHERE proposal_id = $1 AND refinement_stop_tag = 'reaped_phantom' AND created_at::timestamptz >= transaction_timestamp() - interval '24 hours'").bind(proposal_id).fetch_one(self.db().pool()).await?;
        Ok(RefinementLifecycleAggregate { stale_run_count: stale, reaped_phantom_last_24h: reaped })
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ClaimRefinementIntentRequest { pub run_id: String, pub intent_id: String, pub generation: i32, pub owner: String, pub lease_millis: i64 }
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ReleaseRefinementIntentClaimRequest { pub run_id: String, pub intent_id: String, pub generation: i32, pub owner: String }
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct AcknowledgeRefinementTaskMaterializationRequest { pub run_id: String, pub intent_id: String, pub generation: i32, pub task_id: String, pub owner: String }
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CompleteRefinementIntentRequest { pub run_id: String, pub intent_id: String, pub generation: i32, pub owner: String, pub next_round: i32, pub next_phase: RefinementPhase, pub next_role: RefinementRole, pub next_idempotency_key: String }
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RefinementNextIntent { pub intent_id: String, pub round: i32, pub phase: RefinementPhase, pub role: RefinementRole }
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ParkRefinementRunRequest { pub run_id: String, pub generation: i32, pub kind: RefinementParkKind }
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct TerminalRefinementRunRequest { pub run_id: String, pub generation: i32, pub reason: RefinementStopReason }
/// A named durable append boundary that is permitted to move the heartbeat.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum RefinementDurableProgress { DebateAppend, VerdictAppend, TaskStarted }
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RefinementLifecycleAggregate { pub stale_run_count: i64, pub reaped_phantom_last_24h: i64 }

#[derive(Debug, thiserror::Error)]
pub enum RefinementIntentMutationError {
    #[error(transparent)] Database(#[from] Error),
    #[error("invalid refinement intent mutation: {0}")] InvalidRequest(String),
    #[error("run {run_id} generation {generation} is not current")] GenerationConflict { run_id: String, generation: i32 },
    #[error("intent {intent_id} is not claimable by {owner}")] ClaimConflict { intent_id: String, owner: String },
    #[error("task {task_id} does not have the required refinement correlation")] TaskCorrelationMismatch { task_id: String },
}
impl From<sqlx::Error> for RefinementIntentMutationError { fn from(error: sqlx::Error) -> Self { Self::Database(Error::from(error)) } }
...
impl ProposalRepository {
    ...
    pub async fn load_pending_or_claimed_refinement_intents(&self, run_id: &str, generation: i32) -> IntentMutationResult<Vec<RefinementPendingIntent>> {
        self.db().ensure_initialized().await?;
    ...
    pub async fn claim_refinement_intent(&self, request: ClaimRefinementIntentRequest) -> IntentMutationResult<Option<RefinementIntentLease>> {
        valid_intent_request(&request.run_id, &request.intent_id, request.generation, &request.owner)?;
        if request.lease_millis <= 0 { return Err(RefinementIntentMutationError::InvalidRequest("lease must be positive".into())); }
        self.db().ensure_initialized().await?;
    ...
    pub async fn release_refinement_intent_claim(&self, request: ReleaseRefinementIntentClaimRequest) -> IntentMutationResult<bool> {
        valid_intent_request(&request.run_id, &request.intent_id, request.generation, &request.owner)?;
        self.db().ensure_initialized().await?;
    ...
    pub async fn expire_refinement_intent_claims(&self, run_id: &str, generation: i32) -> IntentMutationResult<u64> {
        self.db().ensure_initialized().await?;
...
fn valid_intent_request(run_id: &str, intent_id: &str, generation: i32, owner: &str) -> IntentMutationResult<()> {
    if run_id.trim().is_empty() || intent_id.trim().is_empty() || owner.trim().is_empty() || generation <= 0 { return Err(RefinementIntentMutationError::InvalidRequest("run, intent, owner, and positive generation are required".into())); }
    Ok(())
}
async fn ensure_generation(pool: &sqlx::PgPool, run_id: &str, generation: i32) -> IntentMutationResult<()> {
    let present = sqlx::query_scalar::<_, bool>("SELECT EXISTS(SELECT 1 FROM refinement_runs WHERE id = $1 AND generation = $2)").bind(run_id).bind(generation).fetch_one(pool).await?;
    present.then_some(()).ok_or_else(|| RefinementIntentMutationError::GenerationConflict { run_id: run_id.to_owned(), generation })
}
fn snapshot_to_mutation(error: RefinementRunSnapshotError) -> RefinementIntentMutationError { RefinementIntentMutationError::InvalidRequest(error.to_string()) }
fn pending_intent_row(row: PgRow) -> IntentMutationResult<RefinementPendingIntent> {
    Ok(RefinementPendingIntent { intent_id: row.get("id"), run_id: row.get("run_id"), generation: row.get("generation"), round: row.get("round"), phase: phase(row.get("phase")).map_err(snapshot_to_mutation)?, role: role(row.get("role")).map_err(snapshot_to_mutation)?, state: intent_state(row.get("state")).map_err(snapshot_to_mutation)?, claimed_by: row.get("claimed_by"), claim_expires_at: row.get::<Option<String>, _>("claim_expires_at").map(|v| timestamp(&v)).transpose().map_err(snapshot_to_mutation)? })
}
fn lease_row(row: PgRow) -> IntentMutationResult<RefinementIntentLease> {
    Ok(RefinementIntentLease { intent_id: row.get("id"), run_id: row.get("run_id"), generation: row.get("generation"), round: row.get("round"), phase: phase(row.get("phase")).map_err(snapshot_to_mutation)?, role: role(row.get("role")).map_err(snapshot_to_mutation)?, owner: row.get("claimed_by"), expires_at: timestamp(&row.get::<String, _>("claim_expires_at")).map_err(snapshot_to_mutation)? })
}
