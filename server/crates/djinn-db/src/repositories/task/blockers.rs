...
/// The dependent-release query. Named so
/// [`super::legacy_delivery`] can assert the discriminator it routes on.
pub(super) const EMIT_UNBLOCKED_TASKS_SQL: &str = "SELECT t.id, t.project_id, t.short_id, t.epic_id, t.title, t.description, t.design,
                    ...
                    (SELECT s.primary_blocking_check FROM task_pr_ci_snapshots s WHERE s.task_id = t.id ORDER BY s.last_seen_at DESC LIMIT 1) AS ci_primary_blocking_check,
                    (SELECT s.failure_annotations FROM task_pr_ci_snapshots s WHERE s.task_id = t.id ORDER BY s.last_seen_at DESC LIMIT 1) AS ci_failure_annotations,
                   ...
                   JOIN tasks bt ON bt.id = b2.blocking_task_id
                    WHERE b2.task_id = t.id
                       AND (
                           bt.status != 'closed'
                           OR (
                               -- Explicitly legacy, by label. NOT by
                               -- `pr_url`: that field is nullable and a stray
                               -- value on a canonically direct-owned blocker
                               -- would release dependents on work no ledger
                               -- generation ever applied.
                               NOT (bt.labels @> '[\"direct-delivery-legacy\"]'::jsonb)
                               AND EXISTS (
...
impl TaskRepository {
    ...
    pub(super) async fn emit_unblocked_tasks(&self, completed_task_id: &str) -> Result<()> {
        ...
        let unblocked = sqlx::query_as::<_, Task>(EMIT_UNBLOCKED_TASKS_SQL)
