...
pub(super) async fn stranded_ready_section(pool: &sqlx::PgPool) -> serde_json::Value {
    ...
    let findings: Vec<serde_json::Value> = rows
        ...
        .filter_map(|row| {
            ...
            // Legacy nullable-era rows remain visible as blocked attribution
            // diagnostics; an absent creator can never be credential-ready.
...
pub(super) async fn closed_parent_open_children_section(pool: &sqlx::PgPool) -> serde_json::Value {
    ...
    let rows = sqlx::query(
        r#"WITH terminal_proposals AS (
            ...
            SELECT
              ...
              t.updated_at,
              t.epic_id,
              (SELECT s.id FROM sessions s WHERE s.task_id = t.id AND s.status = 'running' ORDER BY s.started_at DESC LIMIT 1) AS active_session_id,
              e.short_id AS epic_short_id,
              e.status AS epic_status,
    ...
    for row in rows {
        ...
        findings.push(serde_json::json!({
            ...
            "preserved_session_id": row.get::<Option<String>, _>("active_session_id"),
            "preserved_pr_url": row.get::<Option<String>, _>("pr_url"),
