...
const SCAN_STATE_COLUMNS: &str = "cursor_id, eligible_high_water_id, pass_id, pass_started_at, leader_epoch, completed_at, last_pass_duration_ms";
...
impl TaskRepository {
    ...
    pub async fn start_or_resume_board_health_mismatch_pass(
        ...
        let result = if state.eligible_high_water_id.is_none() {
            ...
            .fetch_one(&mut *tx)
            .await?;
            sqlx::query_as(&format!("UPDATE board_health_mismatch_scan_state SET cursor_id=NULL, eligible_high_water_id=$1, pass_id=$2, pass_started_at=to_char(now() AT TIME ZONE 'utc', 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"'), leader_epoch=$3, completed_at=NULL, last_pass_duration_ms=NULL WHERE singleton RETURNING {SCAN_STATE_COLUMNS}"))
    ...
    pub async fn load_board_health_mismatch_page(
        ...
        self.db.ensure_initialized().await?;
        let mut tx = self.db.pool().begin().await?;
        let state: BoardHealthMismatchScanState = sqlx::query_as(&format!("SELECT {SCAN_STATE_COLUMNS} FROM board_health_mismatch_scan_state WHERE singleton FOR UPDATE")).fetch_optional(&mut *tx).await?.ok_or_else(|| Error::InvalidData("board-health mismatch pass has not been started".into()))?;
        ...
        let candidates = match &state.eligible_high_water_id {
            None => Vec::new(),
            Some(high_water) => sqlx::query_as(&format!("SELECT t.id, t.short_id, t.epic_id, t.title, t.description, t.design, t.acceptance_criteria::text AS acceptance_criteria, t.issue_type, t.status, t.total_reopen_count FROM tasks t WHERE {} AND ($1::text IS NULL OR t.id > $1) AND t.id <= $2 ORDER BY t.id ASC LIMIT {}", board_health_mismatch_predicate("t"), BOARD_HEALTH_MISMATCH_PAGE_SIZE)).bind(&state.cursor_id).bind(high_water).fetch_all(&mut *tx).await?,
        };
        tx.commit().await?;
    ...
    pub async fn commit_board_health_mismatch_page(
        ...
        self.db.ensure_initialized().await?;
        sqlx::query_as(&format!("UPDATE board_health_mismatch_scan_state SET cursor_id=$1 WHERE singleton AND leader_epoch=$2 AND cursor_id IS NOT DISTINCT FROM $3 AND eligible_high_water_id IS NOT NULL AND $1 <= eligible_high_water_id RETURNING {SCAN_STATE_COLUMNS}"))
            .bind(page_last_id).bind(leader_epoch).bind(expected_cursor).fetch_optional(self.db.pool()).await?
    ...
    pub async fn complete_board_health_mismatch_pass(
        ...
        let remaining = match &state.eligible_high_water_id { None => false, Some(high_water) => sqlx::query_scalar(&format!("SELECT EXISTS(SELECT 1 FROM tasks t WHERE {} AND ($1::text IS NULL OR t.id > $1) AND t.id <= $2)", board_health_mismatch_predicate("t"))).bind(&state.cursor_id).bind(high_water).fetch_one(&mut *tx).await? };
        if remaining {
            return Err(Error::InvalidData(
                "board-health mismatch pass still has an uncommitted page".into(),
            ));
        }
        let result = sqlx::query_as(&format!("UPDATE board_health_mismatch_scan_state SET cursor_id=NULL, eligible_high_water_id=NULL, completed_at=to_char(now() AT TIME ZONE 'utc', 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"'), last_pass_duration_ms=CASE WHEN pass_started_at IS NULL THEN NULL ELSE (EXTRACT(EPOCH FROM (now() - pass_started_at::timestamptz))*1000)::bigint END WHERE singleton RETURNING {SCAN_STATE_COLUMNS}")).fetch_one(&mut *tx).await?;
        tx.commit().await?;
        Ok(result)
    }
}
