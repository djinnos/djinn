...
impl TaskRepository {
    ...
    pub async fn commit_board_health_mismatch_page(
        ...
        self.db.ensure_initialized().await?;
        sqlx::query_as(&format!("UPDATE board_health_mismatch_scan_state SET cursor_id=$1 WHERE singleton AND active AND leader_epoch=$2 AND cursor_id IS NOT DISTINCT FROM $3 AND eligible_high_water_id IS NOT NULL AND $1 <= eligible_high_water_id RETURNING {SCAN_STATE_COLUMNS}"))
    ...
    pub async fn complete_board_health_mismatch_pass(
        ...
        let result = sqlx::query_as(&format!("UPDATE board_health_mismatch_scan_state SET active=FALSE, cursor_id=NULL, eligible_high_water_id=NULL, completed_at=to_char(now() AT TIME ZONE 'utc', 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"'), last_pass_duration_ms=CASE WHEN pass_started_at IS NULL THEN NULL ELSE (EXTRACT(EPOCH FROM (now() - pass_started_at::timestamptz))*1000)::bigint END WHERE singleton RETURNING {SCAN_STATE_COLUMNS}")).fetch_one(&mut *tx).await?;
        tx.commit().await?;
        Ok(result)
    }
}
