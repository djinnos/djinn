...
pub struct SkipReasonCounts {
    ...
    /// Candidates dropped whole because their fixed per-line overhead alone
    /// exceeded the per-line byte cap. Reported separately from
    /// `budget_pruned` so a silently deleted note stays visible.
    pub oversized_skipped: i64,
...
pub(super) fn build_evidence(
    ...
    RetrievalTraceHealthEvidence {
        ...
        skip_reason_counts: SkipReasonCounts {
            ...
            oversized_skipped: stats.oversized_skipped_count,
...
pub(super) const HEALTH_ROLLUP_TRACE_CANDIDATE_PER_EP_SQL: &str = r#"
    ...
    candidate_stats AS (
        SELECT
            ...
            count(*) FILTER (WHERE c.value->>'skipped_reason' = 'oversized_skipped')::bigint AS oversized_skipped_count,
