...
const MIGRATION_VERSION: u64 = 173;
const MIGRATION_FILE: &str = "173_session_failure_cause.sql";
const MIGRATION_OPERATOR_ID: &str = "00000000-0000-7000-8000-000000000173";
    ...
    conn.execute(
        format!(
            "INSERT INTO users (id, github_id, github_login) VALUES \
             ('{MIGRATION_OPERATOR_ID}', 9000000173, 'failure-cause-migration-operator') \
...
async fn apply_migration_173(conn: &mut PgConnection) {
    let sql = std::fs::read_to_string(migrations_dir().join(MIGRATION_FILE))
        .expect("read migration 173 sql");
    conn.execute(sql.as_str())
        .await
        .expect("apply migration 173 after prior migrations");
}
...
#[tokio::test]
async fn migration_173_applies_on_fresh_database() {
...
#[tokio::test]
async fn migration_173_applies_after_prior_migrations_without_backfill() {
    with_temp_database("failure_cause_prior", |db_url| async move {
        ...
        apply_migration_173(&mut conn).await;
        drop(conn);
