//! Live-Postgres fixture harness for migration 138's creator contract.
//!
...
/// tier below is a durable typed relation consumed by migration 138.
#[tokio::test]
...
async fn unset_zero_task_and_forced_post_update_failure_are_atomic() {
    ...
    assert_eq!(
        sqlx::query_scalar::<_, i64>("SELECT count(*) FROM _sqlx_migrations WHERE version = 138")
            .fetch_one(&mut conn)
            .await
            .unwrap(),
        0
