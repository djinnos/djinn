...
use djinn_db::{
    ...
    repositories::readiness::{
        ...
        ReadinessIdentifiedArea, ReadinessRepository,
    },
};
...
async fn identification_failure_paths_leave_no_area_fanout() {
    let db = Database::ephemeral().await.expect("open postgres test database");
    ...
    let invalid = repo.materialize_kickoff(kickoff_input(project, &creator, "invalid"))
        .await.expect("materialize invalid fixture");
    repo.complete_identification(&invalid.run.id, &creator, ReadinessIdentificationOutput { areas: vec![] })
        .await.expect_err("zero areas fails identification");
    assert_failed_without_fanout(&db, &invalid.run.id, "identification output must contain at least one area").await;

    let explicit = repo.materialize_kickoff(kickoff_input(project, &creator, "explicit"))
        .await.expect("materialize explicit fixture");
    repo.fail_identification(&explicit.run.id, "architect failure")
        .await.expect("explicit failure terminalizes");
...
async fn identification_completion_persists_full_area_task_payloads_and_context() {
    ...
    let repo = ReadinessRepository::new(db.clone());
    let kickoff = repo.materialize_kickoff(kickoff_input(project, &creator, "complete"))
        .await.expect("materialize completion fixture");
    let output = identification_output();
    let fanout = repo.complete_identification(&kickoff.run.id, &creator, output.clone())
        .await.expect("complete valid identification");
    assert_eq!(fanout_counts(&db, &kickoff.run.id).await, (2, 2, 2));
    let run: (String, Option<i32>) = sqlx::query_as("SELECT status,expected_area_count FROM readiness_runs WHERE id=$1")
        .bind(&kickoff.run.id).fetch_one(db.pool()).await.expect("load run");
    ...
    let expected_context = TaskExecutionContext::readiness_guardrail_analysis("agent-readiness-guardrails", "1.0.0").expect("context");
    ...
    for item in &fanout {
        ...
        let description: serde_json::Value = serde_json::from_str(&task.description).expect("task description JSON");
        let identified = output.areas.iter().find(|area| area.area_key == item.area.area_key).expect("matching area");
        ...
        assert_eq!(description["repository_snapshot"], "sha256:readiness-fixture");
        ...
        assert_eq!(description["path_scopes"], serde_json::json!(&identified.path_scopes));
        assert_eq!(description["composition"], composition);
        assert_eq!(item.area.composition, composition);
        assert_eq!(item.area.path_scopes, serde_json::json!(&identified.path_scopes));
...
async fn identification_fanout_database_failure_rolls_back_every_artifact() {
    ...
    let repo = ReadinessRepository::new(db.clone());
    let kickoff = repo.materialize_kickoff(kickoff_input(project, &creator, "rollback"))
        .await.expect("materialize rollback fixture");
    ...
    let error = repo.complete_identification(&kickoff.run.id, &creator, identification_output())
        .await.expect_err("second area aborts transaction");
    assert!(error.to_string().contains("injected readiness fanout failure"));
    assert_eq!(fanout_counts(&db, &kickoff.run.id).await, (0, 0, 0));
    let run: (String, Option<i32>) = sqlx::query_as("SELECT status,expected_area_count FROM readiness_runs WHERE id=$1")
        .bind(&kickoff.run.id).fetch_one(db.pool()).await.expect("load rollback run");
...
fn identification_output() -> ReadinessIdentificationOutput {
    ReadinessIdentificationOutput { areas: vec![
        ReadinessIdentifiedArea { area_key: "frontend".into(), path_scopes: vec!["apps/web/**".into()], languages: vec!["TypeScript".into()], roles: vec!["frontend".into()], frameworks: vec!["React".into()], key_libraries: vec!["vite".into()], confidence: 0.9, evidence: vec!["apps/web/package.json".into()] },
        ReadinessIdentifiedArea { area_key: "backend".into(), path_scopes: vec!["server/**".into()], languages: vec!["Rust".into()], roles: vec!["api".into()], frameworks: vec!["Axum".into()], key_libraries: vec!["sqlx".into()], confidence: 0.95, evidence: vec!["server/Cargo.toml".into()] },
    ] }
}

async fn fanout_counts(db: &Database, run_id: &str) -> (i64, i64, i64) {
    let areas = sqlx::query_scalar("SELECT COUNT(*) FROM readiness_composition_areas WHERE run_id=$1").bind(run_id).fetch_one(db.pool()).await.expect("count areas");
    let attempts = sqlx::query_scalar("SELECT COUNT(*) FROM readiness_area_attempts WHERE run_id=$1").bind(run_id).fetch_one(db.pool()).await.expect("count attempts");
