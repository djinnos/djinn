...
async fn migration_138_backfills_deterministic_intervals_and_stop_vocabulary() {
    temporary("history", |url| async move {
        ...
        let canonical = ["adversary_dry", "round_cap", "spawn_cap", "repeated_objection", "agent_failure", "human_accepted", "human_rejected", "interrupted", "reaped_phantom", "operator_stop"];
        ...
        lifecycle(&pool,"ordinary","p-1","status_change","2025-01-04T00:00:00Z",None).await;
        ...
        let ids: Vec<String> = sqlx::query_scalar("SELECT id FROM refinement_runs ORDER BY source_start_revision_id").fetch_all(&pool).await.unwrap();
        assert!(ids.iter().all(|id| uuid::Uuid::parse_str(id).is_ok()));
        assert_eq!(sqlx::query_scalar::<_,i64>("SELECT count(*) FROM refinement_runs").fetch_one(&pool).await.unwrap(), 14);
        pool.close().await;
    }).await;
}
