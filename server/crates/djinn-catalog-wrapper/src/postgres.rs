//! Postgres catalog-service adapter and its Unix control-socket server.
//!
//! Provisioning and rollback are owned by a background task (never the request
//! future or the lease-map mutex); a retry that finds retained cleanup authority
//! retries cleanup rather than orphaning a tenant.
