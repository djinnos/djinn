...
///
/// Kept alongside [`write_frame`] even though production RPC now flows over
/// TCP via `djinn_runtime::wire::read_frame`.  The stdout write-only helper
/// path lives here for the terminal `TaskRunReport` frame, and the
/// symmetric reader stays so tests and tooling can decode it.
#[allow(dead_code)]
