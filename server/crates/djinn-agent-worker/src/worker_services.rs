...
impl SupervisorServices for WorkerSupervisorServices {
    ...
    /// epic). Worker pods no longer run any verification; the supervisor
    /// always transitions the task straight to `needs_task_review` at the
    /// `WorkerSubmitted` hand-off. This trait method therefore always
    /// returns `Ok(None)` — the same default as the host-side `DirectServices`
    /// — matching the trait's documented "no-op default" semantics.
