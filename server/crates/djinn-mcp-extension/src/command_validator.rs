...
mod tests {
    ...
    fn rejects_curl_post() {
        let violations =
            validate_read_only_command("curl -X POST https://example.com").unwrap_err();
    ...
    fn rejects_wget_post() {
        let violations =
            validate_read_only_command("wget --post-data='foo=bar' https://example.com")
                .unwrap_err();
        assert!(violations.contains(&CommandViolation::NetworkMutation));
    }
    ...
    fn rejects_sed_i_flag() {
        let violations =
            validate_read_only_command("sed -i.bak 's/foo/bar/' file.txt").unwrap_err();
        assert!(violations.contains(&CommandViolation::FileMutation));
    }
    ...
    fn rejects_curl_patch() {
        let violations =
            validate_read_only_command("curl -X PATCH https://example.com").unwrap_err();
        assert!(violations.contains(&CommandViolation::NetworkMutation));
    }
    ...
    fn rejects_curl_delete_method() {
        let violations =
            validate_read_only_command("curl -X DELETE https://example.com").unwrap_err();
        assert!(violations.contains(&CommandViolation::NetworkMutation));
    }
    ...
    fn rejects_redis_hset() {
        let violations =
            validate_read_only_command("redis-cli HSET myhash field value").unwrap_err();
        assert!(violations.contains(&CommandViolation::DatabaseMutation));
    }
