...
//!
//! Copied from `djinn-agent::extension::fuzzy` so extension handlers can
//! use it without depending on `djinn-agent`.
...
/// Re-indent a replacement block to match the indentation of the matched block.
pub(crate) fn reindent_replacement(matched_block: &str, replacement: &str) -> String {
