...
pub fn tool_proposal_reconcile_obsolete_epic() -> RmcpTool {
    RmcpTool::new(
        ...
        "Scoped teardown for one obsolete graduated epic. Blocks if any task has merged work; otherwise applies shared disposition to only the selected epic's children: disposed (closed), parked, retained for another open proposal parent, or retained for an external dependent. It then closes the selected epic and unlinks it from the proposal. Use instead of whole-build proposal_stop_build during Reconcile tasks.".to_string(),
        object!({
            "type": "object",
