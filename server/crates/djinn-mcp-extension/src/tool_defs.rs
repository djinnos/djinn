...
pub fn tool_ci_job_log() -> RmcpTool {
    RmcpTool::new(
        ...
        "Fetch the failing GitHub Actions CI log for this task's PR. Call with NO arguments \
         to auto-discover the currently-failing jobs from the task's recorded CI state — it \
         checks the PR-head lane first, then falls back to the merge-queue (merge_group) lane, \
         so it stays accurate even after a new push makes old job ids stale. When several jobs \
         fail it returns the first job's log with a header listing the others (name + job_id); \
         pass `job_id` to fetch a specific one, and `step` to narrow to a single failed step. \
         `pr_number` targets another PR in the same project's repo (e.g. planner escalation \
         tasks whose description names the source PR). If the output is large, use output_view \
         or output_grep to navigate it."
            .to_string(),
        object!({
            "type": "object",
            "properties": {
                "job_id": {"type": "integer", "description": "Optional. A specific GitHub Actions job ID to fetch directly. Omit to auto-discover the failing jobs."},
                "pr_number": {"type": "integer", "description": "Optional. Target this PR number in the same project's repo instead of the task's own PR (for escalation tasks that reference a source PR)."},
