...
# normalization task-run Pods apply before compiling — then compiles the
...
# worker trees, so each run recompiled cold. See
...
mod tests {
    ...
    fn builds_warm_job_manifest_with_expected_shape() {
        ...
        // mtimes are normalized to match task-run Pods — NOT in this shell wrapper.
