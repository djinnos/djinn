...
//! task-run Pod should provide (the `image_service_presets` junction, migration
//! 66). Each declared service is injected as a *native sidecar*: an init
//! container with `restartPolicy: Always`. A native sidecar
...
//! a project's image (used by task-run dispatch).
