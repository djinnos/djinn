...
///   deterministic private dir (under `/cache/cargo-target-runs/<id>`) seeded
///   from that warm base, so they never write the shared base directly or contend
///   on Cargo's shared build-dir lock, and recompile only their delta
///   incrementally. (Default is <workspace>/target inside the ephemeral clone —
...
fn common_cache_env_vars(project_id: &str) -> Vec<EnvVar> {
    vec![
        ...
        // base with `CARGO_INCREMENTAL=1`, and task-run pods seed a private run
        // target dir from that base and recompile only their delta
        // incrementally. Forcing sccache (which requires CARGO_INCREMENTAL=0)
        // disables incremental and was the wrong lever for edit-loop builds.
        // SCCACHE_DIR is left set below so a repo that *itself* pins
        // `rustc-wrapper = "sccache"` in its .cargo/config.toml still gets a
        // writable, Landlock-allowed cache dir; we just don't impose the wrapper
        // on repos that don't ask for it.
        env_var(
            "SCCACHE_DIR",
        ...
        // Build-in-pod contexts (task-run, warm) have no Postgres reachable,
        // but a repo's .cargo/config.toml may bake a DATABASE_URL for
...
/// base. Warm Pods write this base directly; task-run Pods use private target
/// dirs seeded from this base.
...
/// `rustc-wrapper = "sccache"`), matching `warm_cache_env_vars`. Warm == worker
/// must use the SAME compile strategy or the warm seed is wasted. `policy` is
/// accepted for signature parity but no longer flips incremental.
...
mod tests {
    ...
    #[test]
    fn explicit_policy_no_sccache_enables_incremental_for_warm_and_worker() {
