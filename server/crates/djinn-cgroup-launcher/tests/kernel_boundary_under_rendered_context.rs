...
fn the_privileged_lane_is_wired_and_this_proof_cannot_silently_skip() {
    ...
    assert_eq!(context.get("seccomp_profile"), "RuntimeDefault");
    assert_eq!(context.get("launcher_apparmor_profile"), "RuntimeDefault");
    let residual_capabilities = context.get("launcher_capabilities_add");
    assert_eq!(residual_capabilities, "CHOWN,SETGID,SETUID,SETPCAP");
    assert!(
        !residual_capabilities.contains("SYS_ADMIN")
            && !residual_capabilities.contains("SYS_RESOURCE"),
        "the privilege-free launcher must not receive bootstrap mount capabilities"
    );
    assert_eq!(context.get("launcher_cgroup_root"), "/sys/fs/cgroup");
    assert_eq!(context.get("launcher_cgroup_source"), "runtimeclass");
    assert_eq!(context.get("launcher_cgroup_volume"), "none");
    assert!(
        !context.contains("launcher_bootstrap_only_capabilities"),
        "the RuntimeClass cutover must not retain a post-bootstrap capability-drop contract"
    );
    assert_eq!(
