...
//!
//! # The second `[include]`: private-dependency auth (goxi, ninth blocker)
//!
//! `SYSTEM scope is purely additive` was true of `/etc/gitconfig` and false of
//! `$HOME/.gitconfig` the moment a command started executing in a *different
//! container* than the one that wrote it. `djinn_agent_worker`'s
//! `configure_private_dep_access` writes the installation token into the
//! WORKER's `$HOME/.gitconfig`; the launcher's `$HOME` is a different volume, so
//! a brokered fetch of a private dependency went out unauthenticated and
//! silently (GitHub answers a private repo 404, which reads as "not found").
//!
//! The token therefore has to cross the container boundary, and it does so on a
//! one-way channel: an `emptyDir` the worker mounts read-write and the launcher
//! mounts `readOnly: true`, named to both by
//! [`CHILD_GIT_CONFIG_PATH_ENV`]. This module includes that file when the
//! environment names it. Three properties make that safe to do at *protected*
//! scope:
//!
//! * the path comes from the launcher's OWN process environment — the rendered
//!   container env — never from a caller's `CommandSpec`, so it is not the
//!   injection primitive the rest of this module exists to keep shut;
//! * the file is on a mount the kubelet makes read-only in this container, so
//!   neither the launcher nor its child can rewrite it into `core.sshCommand`;
//! * an absent file makes git ignore the include, so a pod rendered without the
//!   channel degrades to today's behaviour rather than failing to spawn.
//!
//! `$HOME/.gitconfig` is still not redirected. `GIT_CONFIG_GLOBAL` remains
//! unforwardable and unset, exactly as `#2617` left it.
//!
...
/// Env var naming the worker-written, launcher-read-only git config holding the
/// project's private-dependency `url.<...>.insteadOf` rewrite.
///
/// Must equal `djinn_k8s::private_dep_config::CHILD_GIT_CONFIG_PATH_ENV`; the
/// launcher does not depend on the renderer, and the guard in
/// `djinn-k8s/tests/launcher_child_runtime_handoff.rs` asserts the two agree.
pub const CHILD_GIT_CONFIG_PATH_ENV: &str = "DJINN_CHILD_GIT_CONFIG_PATH";

/// The private-dependency config to `[include]`, from this process's OWN
/// environment.
///
/// Deliberately not a parameter, and deliberately not read from a
/// `CommandSpec`: the value is the rendered container env of the privileged
/// launcher, so a caller cannot reach it. A relative value is refused rather
/// than resolved, because git would interpret it relative to the *including*
/// file's directory — the anchor directory — and silently pick up something
/// else.
fn private_dep_config_include() -> Option<PathBuf> {
    private_dep_config_include_from(std::env::var_os(CHILD_GIT_CONFIG_PATH_ENV))
}

/// [`private_dep_config_include`]'s decision, as a pure function of the value.
///
/// Split out so the acceptance rule can be tested without mutating the process
/// environment: unit tests share one process and `materialize_in` reads this
/// variable, so an env-mutating test would flake every other test in the binary.
fn private_dep_config_include_from(value: Option<std::ffi::OsString>) -> Option<PathBuf> {
...
/// Body of the generated system-scope config, chaining to each of `includes`.
///
/// Order is the caller's. git ignores an include whose target does not exist, so
/// a path that has not been written yet — the private-dependency config is
/// produced by the worker at task-run startup, possibly after the anchor is
/// first materialized — costs nothing and starts working when it appears.
pub fn anchor_contents(includes: &[PathBuf]) -> String {
    ...
    for include in includes {
        ...
        contents.push_str(&quote_config_value(include));
        contents.push('\n');
    }
...
/// Every file the generated anchor chains to, in application order.
///
/// The real system config first, so nothing the image put in `/etc/gitconfig`
/// is lost to `GIT_CONFIG_SYSTEM` replacing it; then the private-dependency
/// config, which is the one this pod adds.
fn includes_for(generated: &Path, system_config: Option<&Path>) -> Vec<PathBuf> {
    system_config
        .map(Path::to_path_buf)
        .into_iter()
        .chain(private_dep_config_include())
        .filter(|include| include != generated)
        .collect()
}
...
pub fn materialize_in_with_system(
    ...
    let path = anchor_dir_in(root)?.join(ANCHOR_BASENAME);
    let includes = includes_for(&path, system_config);
    write_anchor(&path, &anchor_contents(&includes))?;
    Ok(path)
}

/// [`materialize_in_with_system`], with the private-dependency include supplied
/// explicitly instead of read from [`CHILD_GIT_CONFIG_PATH_ENV`].
///
/// Exists so a behavioural test can exercise the real writer over a real channel
/// file **without mutating the process environment**. That is not fastidiousness:
/// `materialize_in` reads that variable, unit tests in a single binary share one
/// process, and this repo already carries a flake note about a test that raced
/// another through shared global state.
pub fn materialize_in_with(
    root: &Path,
    system_config: Option<&Path>,
    private_dep_config: Option<&Path>,
) -> io::Result<PathBuf> {
    let path = anchor_dir_in(root)?.join(ANCHOR_BASENAME);
    let includes: Vec<PathBuf> = system_config
        .into_iter()
        .chain(private_dep_config)
        .filter(|include| *include != path.as_path())
        .map(Path::to_path_buf)
        .collect();
    write_anchor(&path, &anchor_contents(&includes))?;
    Ok(path)
...
pub fn anchor_path() -> Result<&'static Path, Error> {
    ...
    if !anchor_is_intact(path) {
        let chain = system_config_to_chain(path);
        let contents = anchor_contents(&includes_for(path, chain.as_deref()));
...
mod tests {
    ...
    fn the_anchor_chains_to_the_real_system_config_when_one_exists() {
        let chained = anchor_contents(&[PathBuf::from("/etc/gitconfig")]);
        assert!(chained.contains("[include]"));
        assert!(chained.contains("path = \"/etc/gitconfig\""));
        assert!(
            !anchor_contents(&[]).contains("[include]"),
            "no include when there is nothing to chain to"
        );
    }

    /// The private-dependency config has to be chained IN ADDITION to the real
    /// system config, not instead of it: dropping `/etc/gitconfig` is what
    /// `GIT_CONFIG_SYSTEM` replacing it already threatened, and dropping the
    /// private-dep file is the ninth blocker.
    #[test]
    fn both_chains_are_emitted_and_the_generated_file_never_includes_itself() {
        let generated = PathBuf::from("/tmp/djinn-launcher-git-0/gitconfig");
        let system = PathBuf::from("/etc/gitconfig");
        let private_dep = PathBuf::from("/var/run/djinn/child-git/gitconfig");
        let contents = anchor_contents(&[system, private_dep]);
        let includes: Vec<&str> = contents
            .lines()
        ...
        assert_eq!(
            ...
            vec![
                "\tpath = \"/etc/gitconfig\"",
                "\tpath = \"/var/run/djinn/child-git/gitconfig\"",
            ],
            "both chains, system first"
        );

        // A render that pointed the channel at the anchor itself would produce
        // an include cycle; `includes_for` drops it.
        assert!(
            includes_for(&generated, Some(&generated)).is_empty(),
            "the anchor must never include itself"
        );
    }

    /// The include path comes from the launcher's OWN environment and must be
    /// absolute: git resolves a relative include against the *including* file's
    /// directory, which is the anchor dir, not the mount.
    #[test]
    fn a_relative_private_dep_path_is_refused_rather_than_resolved() {
        use std::ffi::OsString;
        assert_eq!(
            private_dep_config_include_from(Some(OsString::from("relative/gitconfig"))),
            None
        );
        assert_eq!(private_dep_config_include_from(Some(OsString::new())), None);
        assert_eq!(private_dep_config_include_from(None), None);
        assert_eq!(
            private_dep_config_include_from(Some(OsString::from(
                "/var/run/djinn/child-git/gitconfig"
            ))),
            Some(PathBuf::from("/var/run/djinn/child-git/gitconfig"))
        );
    }
