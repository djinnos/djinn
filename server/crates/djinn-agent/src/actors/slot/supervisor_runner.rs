...
pub(crate) async fn run_supervisor_dispatch(
    ...
    if let Some(reason) = infra_death.as_deref() {
        ...
        // Pre-PR verification pipeline was removed (Wave 1 of the sehj
        // epic). The legacy "infra death re-arms verification" path is
        // gone — there is no verification row to look up and no host
        // pipeline to re-arm. We simply fall through to the dispatch
        // match below.
    }
    ...
    match (report_result, teardown) {
        (Ok(streamed), Ok(teardown_report)) => {
            ...
            // Pre-PR verification pipeline was removed (Wave 1 of the sehj
            // epic). The worker stage's `WorkerSubmitted` outcome no longer
            // arms a host-side verification pipeline — the supervisor
            // transitions the task to `needs_task_review` directly. We
            // ignore `report.verification_run_id` and the WorkerSubmitted
            // arming path here.
            let _ = &report.verification_run_id;
