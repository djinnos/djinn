...
impl SessionBudgetRole {
    ...
    fn defaults(self) -> RoleBudgetDefault {
        match self {
            // Workers perform the broadest implementation loops, so they retain
            // the historical step budget while token budget scales with the
            // resolved model context window.
            Self::Worker => RoleBudgetDefault {
                max_turns: 1000,
                cumulative_context_windows: 24,
            },
            // Planners should converge sooner than workers and generally spend
            // fewer full-context passes decomposing/routing work.
            Self::Planner => RoleBudgetDefault {
                max_turns: 600,
                cumulative_context_windows: 12,
            },
            // Architects are focused advisory sessions; keep both turns and
            // cumulative spend tighter than planner/worker defaults.
            Self::Architect => RoleBudgetDefault {
                max_turns: 400,
                cumulative_context_windows: 8,
            },
            // Unknown/specialist roles get worker-like headroom so adding a new
            // role cannot unexpectedly starve production sessions before a
            // role-specific budget is introduced.
