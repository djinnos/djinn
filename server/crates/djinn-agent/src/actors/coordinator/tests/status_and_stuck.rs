...
// (stuck_verifying_with_terminal_inpod_run_is_rearmed_not_released removed —
//  the `verifying` task status was removed from the core model; the re-arm path
//  can no longer transition verifying → needs_task_review via the state machine.)
