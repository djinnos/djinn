...
mod inflight_ledger_tests {
    ...
    const WND1_DISPATCH_SETTLE_TIMEOUT: Duration = Duration::from_secs(2);
                        ...
                        move |task_id, _project_path, _model_id, _app_state, kill, _pause| {
                            ...
                            Box::pin(async move {
                                ...
                                tokio::select! {
                                    _ = release_rx => {}
                                    _ = kill.cancelled() => {}
                                    _ = tokio::time::sleep(WND1_DISPATCH_SETTLE_TIMEOUT) => {}
                                }
                                Ok(())
                            })
                        },
    ...
    async fn wnd1_dispatch_race_harness_never_exceeds_caps_1_through_5() {
        for cap in 1..=5 {
            ...
            assert_eq!(
                actor.dispatched,
                u64::from(cap),
                "cap {cap}: overlapping DB+ledger count must not dispatch extra tasks"
            );
            ...
            while actor.dispatched < WND1_READY_TASK_COUNT as u64 {
                ...
                assert_eq!(
                    actor.dispatched, before_raced_pass,
                    "cap {cap}: completion without pool settlement must still be held by in-flight ledger"
                );
                ...
                assert_eq!(
                    actor.dispatched,
                    before + 1,
                    "cap {cap}: exactly one replacement dispatch should fit after one settlement"
                );
                let task_id = wnd1_recv_started(&mut started_rx).await;
                active_tasks.push(task_id.clone());
                let session_id = materialize_wnd1_running_session(&db, &fixture, &task_id).await;
