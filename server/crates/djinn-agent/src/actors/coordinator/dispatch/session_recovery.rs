...
impl CoordinatorActor {
    ...
    ///
    /// The pre-PR verification pipeline (and the `verifying` task status) was
    /// removed in Wave 1 of the sehj epic, so there is no per-task
    /// verification tracker to consult any more — the slot-pool + session
    /// presence check below is the sole "is the task still in flight?" gate.
    pub(in crate::actors::coordinator) async fn detect_and_recover_stuck_filtered(
        ...
        for status in ["in_progress", "in_task_review", "in_lead_intervention"] {
            ...
            for task in tasks {
                ...
                // work (merge, transition) in a background task. Without the
                // pre-PR verification pipeline, there is no separate
                // background-work tracker to consult — the slot-pool check
                // above is sufficient.
