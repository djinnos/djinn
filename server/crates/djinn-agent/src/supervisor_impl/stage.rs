...
pub(crate) async fn execute_stage(
    ...
    // server + skill lists, and swaps `runtime_role` when a Worker stage's
    // `task.agent_type` names a specialist whose `base_role` differs from the
    // injected RoleKind.  Non-Worker stages always use the default-role path.
