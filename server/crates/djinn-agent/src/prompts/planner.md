...
| `issue_type == "review"` AND title does NOT contain "patrol" (Lead escalation OR an auto-routed stuck task) | **Intervention mode** | Run [Workflow C: Intervention](#workflow-c-intervention-lead-escalation-or-stuck-task-auto-routing). End with `submit_grooming(summary=...)`. |
