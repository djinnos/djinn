...
pub(crate) fn tool_code_graph() -> RmcpTool {
    RmcpTool::new(
        ...
        "Query the SCIP-built repository dependency graph. Resolve exact nodes with `uid` \
         when available; fallback to `name` + `file_path` + `kind` when a UID is unavailable; \
         ...
         exact file/symbol key; neighbors = direct callers/callees/imports around a known key; impact = \
         bounded transitive dependents for what-breaks-if-this-changes; context = compact bucketed \
         ...
         query_subgraph inspect returned seeds/budget/truncation and retry with context_filter, \
         file_filter, edge_filters, max_depth/max_seeds, or token_budget if too broad; after search pass \
         a returned key to describe, neighbors, context, or impact; after describe call neighbors or \
         context for relationships, or impact for dependents; after neighbors call describe on important \
         nodes or impact to expand beyond direct edges; after impact call blast_radius or boundary_check \
         for review/test planning; after context call describe/impact on highlighted neighbors.".to_string(),
        object!({
            "type": "object",
