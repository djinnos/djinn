...
pub(crate) fn tool_code_graph() -> RmcpTool {
    RmcpTool::new(
        ...
        "Query the SCIP-built repository dependency graph. Use `capabilities` for \
         ops/params/defaults; `search` to find candidate keys; `describe`/`context` \
         to inspect one node; `neighbors` for direct edges; `impact`/`blast_radius` for \
         dependents/review planning; `query_subgraph` for a token-budgeted NL \
         slice with narrowing hints. Node resolvers accept `uid` (preferred stable exact input) \
         or `name` + `file_path` + `kind` when a UID is unavailable; ambiguous names return \
         ranked candidates. Traversal/coupling triage controls include `limit`, `offset`, \
         `pageLimit`, `summaryOnly`, and `byDepthCounts`. Partial pages and capped summaries \
         are triage views: absence from a page or summary is NOT evidence that a node, edge, \
         or pair is absent from the full graph. `workspace` scopes listing/bounded ops and \
         only seed resolution for traversal ops; empty workspace is omitted, unknown non-empty \
         workspace may return `workspace_hint` candidates. Follow search/describe results by \
         reusing returned `uid`/`key` in describe, neighbors, context, or impact."
            .to_string(),
        object!({
            "type": "object",
