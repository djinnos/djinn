...
///
/// `worktree_path` identifies the live session for destructive-command gates.
pub(crate) async fn gate_guard_shell_check(
    ...
    match classify_destructive_shell_command(command) {
        // Ordinary commands, including builds that formerly diverged from a
        // verification profile, flow normally.
...
mod tests {
    ...
    #[tokio::test]
    async fn divergent_build_command_flows_normally() {
        let (worktree, state) = setup_worktree("gg-build-pass-");

        gate_guard_shell_check(
            &state,
            Some("worker"),
            worktree.path(),
            "cargo build -p formerly-divergent-package",
        )
        .await
        .expect("ordinary build commands must not be steered toward verification");
    }
