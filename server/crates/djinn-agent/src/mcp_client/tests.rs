...
async fn list_resources_timeout_returns_deterministic_error_when_all_fail() {
    // Build a resource-capable routing state with a test dispatch that hangs.
    // Since list_resources uses the real peer (not test_dispatch), we can't
    // directly test timeout with test_dispatch. Instead, verify the
    // non-fatal behavior: if no server succeeds, return Err.
    //
    // This test validates the "all fail" deterministic error path by checking
    // that a registry with resource servers but no actual peers returns Ok([])
    // (since there are no peers to try, the snapshot is empty).
    let mut resource_servers = HashSet::new();
