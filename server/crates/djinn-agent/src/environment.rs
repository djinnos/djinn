//! Fetch a project's `environment_config` JSON column from Dolt and run
//! pre-verification setup hooks against it.
//!
//! This is the post-P8 replacement for the `.djinn/settings.json` reader
//! that used to live alongside the verification pipeline. The
//! `environment_config` column carries a typed
//! [`djinn_stack::environment::EnvironmentConfig`] and is the single source
//! of truth for setup hooks, MCP defaults, skill globals, and code-graph
//! configuration. The legacy `verification` table no longer exists; the
//! pipeline gates were removed (Wave 1 of the sehj epic), so this module
//! is now the only consumer of the column for project setup purposes.
//!
//! ## Lookup modes
//!
//! The fetch helpers accept either a project id (preferred — the in-process
//! caller already has it) or a workspace path (used by callers that only
//! have the path of the worktree they're rendering). The path form
//! reverse-parses the canonical `{projects_root}/{owner}/{repo}` clone-path
//! shape and looks the project up by GitHub coords via
//! [`djinn_db::ProjectRepository::get_by_github`].
//!
//! ## Soft-failure policy
//!
//! Every edge case — missing row, `'{}'` (pre-reseed) column, malformed
//! JSON, forward-incompatible `schema_version` — degrades to
//! [`djinn_stack::environment::EnvironmentConfig::empty`]. Callers extract
//! the sub-field they need (setup hooks, MCP defaults, etc.) from the
//! returned config. We log a `warn` so misconfiguration is visible in the
//! logs without blocking the task.
...
pub(crate) async fn resolve_project_id_for_path(
    db: &Database,
    worktree_path: &Path,
) -> Option<String> {
...
/// MCP default resolution, skills, and the pre-verification setup-hook
/// runner; callers extract whichever sub-field they need from the returned
/// config.
