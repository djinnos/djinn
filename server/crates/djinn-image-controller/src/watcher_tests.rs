//! Unit tests for [`crate::watcher`].
//!
//! Split into a sibling file (the `#[path]` convention `run_dirs_tests.rs`
//! and `graph_warmer_tests.rs` already use) so `watcher.rs` stays inside the
//! repository's Rust source-size guard. It remains a child module of
//! `watcher`, so it reaches that module's private sentinel parsers directly.
