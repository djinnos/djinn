...
mod tests {
    ...
    fn revision_caller_context() -> TrustedRevisionCallerContext {
        TrustedRevisionCallerContext::authenticated_human("memory-writes-tests")
            .expect("test revision caller must be valid")
    }

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn non_mergeable_note_type_bypasses_dedup() {
        REVISION_CALLER_CONTEXT
            .scope(Some(revision_caller_context()), async {
            ...
            .scope(Some(revision_caller_context()), async {
                ...
                let Json(created1) = server
                    .memory_write(Parameters(WriteParams {
                        ...
                        content:
                            "Use tokio::spawn for concurrent task execution in Rust async code."
                                .to_string(),
                ...
                assert!(
                    created1.error.is_none(),
                    "first pattern write failed: {:?}",
                    created1.error
                );
            ...
            .scope(Some(revision_caller_context()), async {
                ...
                let Json(pattern) = server
                    .memory_write(Parameters(WriteParams {
                        ...
                        content: "Use Result types for explicit error handling in Rust."
                            .to_string(),
                        note_type: "pattern".to_string(),
                        status: None,
                        tags: None,
            ...
            .scope(Some(revision_caller_context()), async {
                ...
                let Json(created) = server
                    .memory_write(Parameters(WriteParams {
                        ...
                        content: "A completely unique pattern that has no dedup candidates."
                            .to_string(),
                        note_type: "pattern".to_string(),
                        status: None,
                        tags: None,
                ...
                let snapshot = metrics.snapshot().expect("metrics snapshot");
                let empty =
                    snapshot.aggregate(RetrievalEntryPoint::JitPitfalls, RetrievalOutcome::Empty);
                assert_eq!(
                    empty.count, 1,
    ...
    async fn memory_write_dedup_records_error_on_missing_candidate() {
        REVISION_CALLER_CONTEXT
            .scope(Some(revision_caller_context()), async {
                ...
                let snapshot = metrics.snapshot().expect("metrics snapshot");
                let error =
                    snapshot.aggregate(RetrievalEntryPoint::JitPitfalls, RetrievalOutcome::Error);
                assert_eq!(
                    error.count, 1,
