...
impl DjinnMcpServer {
    ...
    pub(super) async fn code_graph_shape_check(
        ...
        if params.route_id.is_none() && (params.method.is_none() || params.path.is_none()) {
            return Err("shape_check requires route_id or both method and path".to_string());
        }
    ...
    pub(super) async fn code_graph_api_impact(
        ...
        if params.route_id.is_none() && (params.method.is_none() || params.path.is_none()) {
            return Err("api_impact requires route_id or both method and path".to_string());
        }
        let limit = params.limit.unwrap_or(50).max(0) as usize;
        let api_impact = self
            .state
