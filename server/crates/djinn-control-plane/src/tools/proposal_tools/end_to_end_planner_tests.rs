...
async fn refinement_loop_revisions_carry_visual_spec_attribution() {
    ...
    for (idx, rev) in revisions.iter().enumerate().skip(1) {
        ...
        assert!(
            meta["range_end_byte"].as_u64().unwrap() > meta["range_start_byte"].as_u64().unwrap(),
...
async fn refinement_loop_enriched_proposal_exports_as_valid_mdx() {
    ...
    let original_blocks = parse_mdx_blocks(&stored.body).expect("stored body must parse as MDX");
    let exported_body = mdx
        .splitn(3, "---")
        .nth(2)
        .expect("exported MDX must have a body section after frontmatter")
        .trim_start_matches('\n');
    let exported_blocks = parse_mdx_blocks(exported_body).expect("exported body must parse as MDX");
    assert_eq!(
...
async fn refinement_loop_end_to_end_ties_all_y4td_surfaces_together() {
    ...
    let revisions: Vec<&Value> = shown
        ...
        .filter(|rev| rev.get("event_kind").and_then(|v| v.as_str()) == Some("spec_revision"))
        .collect();
    assert_eq!(revisions.len(), 4, "1 seed + 3 patches = 4 spec revisions");
    for (idx, rev) in revisions.iter().enumerate().skip(1) {
