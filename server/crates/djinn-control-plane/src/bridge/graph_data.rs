...
// ── Crate Graph ────────────────────────────────────────────────────────────────
// Bridge types for the `crate_graph` (workspace_deps) operation. Nodes are
// workspace crates; edges are aggregated cross-crate references.

/// Request for the `crate_graph` bridge operation. Currently empty — the
/// crate graph is always the full workspace view; project context is
/// supplied via [`ProjectCtx`].
#[derive(Debug, Clone, Default)]
