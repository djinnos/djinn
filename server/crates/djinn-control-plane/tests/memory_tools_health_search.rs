...
async fn call_memory_tool(
    ...
    REVISION_CALLER_CONTEXT
        .scope(
            TrustedRevisionCallerContext::authenticated_human("memory-health-search-test"),
...
async fn mcp_memory_health_orphans_and_broken_links_shapes() {
    ...
    let write_error = call_memory_tool(
        &harness,
        "memory_write",
        json!({"project": project, "title": "Source", "content": "[[Missing Target]]", "type": "reference", "reason": "seed health shape fixture"}),
    )
    .await
...
async fn no_regression_memory_search_ranking_notes_only() {
    ...
    call_memory_tool(
        &harness,
        "memory_write",
        json!({"project": project, "title": "Rust Note One", "content": "rust memory test", "type": "reference", "reason": "seed first search fixture"}),
    )
        .await
    ...
    call_memory_tool(
        &harness,
        "memory_write",
        json!({"project": project, "title": "Rust Note Two", "content": "another rust note", "type": "adr", "reason": "seed second search fixture"}),
    )
        .await
