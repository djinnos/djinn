...
use djinn_graph::canonical_graph::{cache_reuse_enabled_from_var, ensure_canonical_graph};
...
/// per-test indexer mutex.  Kept local to this integration test so the
/// production crate does not need to expose a test-support feature just to
/// drive the warm API here.
...
/// that produced it.
pub async fn assert_incremental_matches_full(
    project_id: &str,
    project_root: &Path,
    commit_sha: &str,
    ctx: &TestWarmContext,
...
async fn drive_two_warms(
    ...
    let _first = ensure_canonical_graph(ctx, project_id, project_root, ArchitectWarmToken::new())
        .await
    ...
    djinn_graph::canonical_graph::clear_test_caches().await;
    ...
    let _second = ensure_canonical_graph(ctx, project_id, project_root, ArchitectWarmToken::new())
        .await
...
async fn assert_incremental_matches_full_reports_diff_when_file_added_between_warms() {
    ...
    let first = ensure_canonical_graph(&ctx, &project.id, &project_root, ArchitectWarmToken::new())
        .await
        .expect("first warm must succeed (cache-hit on pre-seeded blob)");
    ...
    djinn_graph::canonical_graph::clear_test_caches().await;
    let _second =
        ensure_canonical_graph(&ctx, &project.id, &project_root, ArchitectWarmToken::new())
            .await
...
fn cache_reuse_seam_defaults_to_disabled_and_honours_env_flag() {
    ...
    assert_eq!(
        djinn_graph::canonical_graph::DJINN_GRAPH_CACHE_REUSE_FLAG,
        "DJINN_GRAPH_CACHE_REUSE"
    );
}
