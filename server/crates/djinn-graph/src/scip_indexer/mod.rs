...
impl SupportedIndexer {
    ...
    /// Build the argv (excluding the binary itself) for one planned
    /// invocation.
    ///
    /// `config_path` is the generated indexer config file, when the planner
    /// produced one. Only [`Self::RustAnalyzer`] consumes it — every other
    /// indexer ignores the parameter entirely, and the planner never produces
    /// a config for them, so their argv is untouched by this seam.
    fn command_args(self, output_path: &Path, config_path: Option<&Path>) -> Vec<String> {
        let output = output_path.to_string_lossy().into_owned();
        match self {
            // `--config-path` carries the Cargo feature selection so the SCIP
            // pass resolves the same feature set as the warm cargo base. Without
            // it rust-analyzer analyses with default features, and resolver-2
            // unification gives every shared dependency a different `-C
            // metadata` than the warm base — so the prelude reuses nothing.
            // See `super::rust_analyzer_config` for the file shape.
            Self::RustAnalyzer => {
                let mut args = vec![
                    "scip".to_string(),
                    ".".to_string(),
...
/// A JSON configuration file generated by the planner and handed to the
/// indexer subprocess via a CLI flag (`rust-analyzer scip --config-path`).
///
/// The plan carries the file's **contents** as well as its path because the
/// two are used for different jobs: the path goes into the argv, and the
/// contents are what the SCIP cache key digests. Keying on content rather than
/// path is what keeps the cache stable — see `cache::CommandShape::from_plan`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct IndexerConfigFile {
    /// Absolute path the `--config-path` argument points at. Derived from the
    /// output root and workspace slug, never randomised.
    pub path: PathBuf,
    /// Exact bytes written to `path` immediately before the invocation.
    pub contents: String,
}
...
pub struct PlannedIndexerCommand {
    ...
    /// Generated indexer config file this invocation must have on disk, if
    /// any. `None` for every indexer except rust-analyzer, and for
    /// rust-analyzer whenever the project declared no Cargo features — which
    /// keeps the planned argv (and therefore the SCIP cache key) identical to
    /// the pre-feature-parity one. `#[serde(default)]` keeps older persisted
    /// plans loadable.
    #[serde(default)]
    pub config_file: Option<IndexerConfigFile>,
}
...
mod build_command_tests {
    ...
    /// Without a config the rust-analyzer argv is exactly what it has always
    /// been — the feature-parity seam is inert by default.
    #[test]
    fn rust_analyzer_args_are_unchanged_without_a_config() {
        assert_eq!(
            SupportedIndexer::RustAnalyzer.command_args(Path::new("/out/index.scip"), None),
            vec!["scip", ".", "--output", "/out/index.scip"]
        );
    }

    /// With a config the flag is appended (never inserted mid-argv, so the
    /// existing positional `.` and `--output` stay where they were).
    #[test]
    fn rust_analyzer_args_append_config_path() {
        assert_eq!(
            SupportedIndexer::RustAnalyzer.command_args(
                Path::new("/out/index.scip"),
                Some(Path::new("/out/.indexer-config/server-rust-analyzer.json")),
            ),
            vec![
                "scip",
                ".",
                ...
                "/out/index.scip",
                "--config-path",
                "/out/.indexer-config/server-rust-analyzer.json",
            ]
        );
    }

    /// Every non-Rust indexer ignores the config parameter entirely. The
    /// planner never hands them one, but pin it here too so a future refactor
    /// cannot start injecting a rust-analyzer flag into scip-typescript.
    #[test]
    fn non_rust_indexers_ignore_the_config_path() {
        let config = Path::new("/out/.indexer-config/server-rust-analyzer.json");
        for indexer in SupportedIndexer::ALL {
            if indexer == SupportedIndexer::RustAnalyzer {
                continue;
            }
            let output = Path::new("/out/index.scip");
            assert_eq!(
                indexer.command_args(output, Some(config)),
                indexer.command_args(output, None),
                "{indexer:?} argv must not depend on the indexer config"
            );
        }
    }
}
