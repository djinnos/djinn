...
mod tests {
    ...
    /// Measurement harness (proposal qoxm AC): walk THIS repo's real git
    /// history, aggregate co-change file pairs exactly as the ingest pipeline
    /// does (`--no-merges`, ≤15 files/commit), then run the full score-floor +
    /// per-file top-K cap and report the resulting edge count. Ignored by
    /// default — run with:
    ///   cargo test -p djinn-graph --lib measure_cochange_edges_on_repo -- --ignored --nocapture
    #[test]
    #[ignore = "measurement harness; walks repo git history, run with --ignored --nocapture"]
    fn measure_cochange_edges_on_repo() {
        use crate::repo_graph::{
            RepoDependencyGraph, RepoGraphArtifact, RepoGraphNode, RepoGraphNodeKind, RepoNodeKey,
            REPO_GRAPH_ARTIFACT_VERSION, RouteExclusionConfig,
        };
        ...
        let out = Command::new("git")
            .args([
                ...
                "--no-merges",
                "--name-only",
                &format!("--pretty=format:{SENTINEL}%cI"),
            ])
            .output()
            .expect("git log");
        let text = String::from_utf8_lossy(&out.stdout);

        // Parse per-commit (date, files).
        ...
        for line in text.lines() {
            if let Some(rest) = line.strip_prefix(SENTINEL) {
                if let Some(d) = cur_date.take() {
                    commits.push((d, std::mem::take(&mut cur_files)));
                }
                cur_date = Some(rest.to_string());
            } else if !line.trim().is_empty() {
                cur_files.push(line.trim().to_string());
            }
        }
        if let Some(d) = cur_date.take() {
            commits.push((d, cur_files));
        }

        // Aggregate co-change counts + latest date per canonical (a<b) pair.
        ...
        for (date, files) in &commits {
            ...
            uniq.sort();
            uniq.dedup();
            // Mirror MAX_FILES_PER_COMMIT_FOR_PAIRS = 15 from the ingest path.
            if uniq.len() < 2 || uniq.len() > 15 {
                continue;
            }
            for i in 0..uniq.len() {
                for j in (i + 1)..uniq.len() {
                    let (a, b) = (uniq[i].clone(), uniq[j].clone());
                    let key = if a < b { (a, b) } else { (b, a) };
                    let e = counts.entry(key).or_insert((0, String::new()));
                    e.0 += 1;
                    if date > &e.1 {
                        e.1 = date.clone();
                    }
        ...
        // Build a graph containing a file node for every file that appears in
        // any pair. (Production intersects these with SCIP-indexed nodes, so
        // the real materialized count is ≤ this measurement.)
        let mut file_set: HashSet<String> = HashSet::new();
        for (a, b) in counts.keys() {
            file_set.insert(a.clone());
            file_set.insert(b.clone());
        }
        let nodes: Vec<RepoGraphNode> = file_set
            .iter()
        ...
        let inputs: Vec<CoChangeInput> = counts
            .iter()
            .map(|((a, b), (n, date))| CoChangeInput {
                file_a: a.clone(),
                file_b: b.clone(),
                co_changes: *n,
                last_co_change_iso: date.clone(),
            })
            .collect();

        let qualifying = inputs
            .iter()
            .filter(|i| i.co_changes >= COCHANGE_MIN_CO_CHANGES)
            .count();
        let edges = derive_cochange_edges(&graph, &inputs);

        eprintln!(
            "qoxm co-change measurement: commits={} distinct_pairs={} pairs>=floor({})={} files={} \
             edges_after_topK({})={}",
            commits.len(),
            counts.len(),
            COCHANGE_MIN_CO_CHANGES,
            qualifying,
            file_set.len(),
            COCHANGE_TOP_K_PER_FILE,
            edges.len(),
        );
    }
}
