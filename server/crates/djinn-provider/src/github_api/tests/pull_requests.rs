...
async fn close_pull_request_success() {
    ...
    let pr = client
        .close_pull_request("djinnos", "server", 42)
        .await
        .expect("close_pull_request should succeed");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn close_pull_request_idempotent_already_closed() {
    // GitHub returns the PR unchanged when you PATCH {"state": "closed"} on an
    // already-closed PR — it is a no-op success (200), not an error. This
    // verifies the caller gets Ok back without needing to check state first.
    let server = MockServer::start().await;
    let install_id = seed_installation_token();

    Mock::given(method("PATCH"))
        .and(path("/repos/djinnos/server/pulls/43"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "number": 43,
            "title": "stale PR",
            "state": "closed",
            "merged": false,
            "html_url": "https://github.com/djinnos/server/pull/43",
            "head": { "ref": "task/stale", "sha": "sha1" },
    ...
    let client = GitHubApiClient::for_installation_with_base_url(install_id, server.uri());
    let pr = client
        .close_pull_request("djinnos", "server", 43)
        .await
        .expect("closing an already-closed PR should succeed");

    assert_eq!(pr.state, PrState::Closed);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn close_pull_request_failure_returns_typed_envelope() {
    let server = MockServer::start().await;
    let install_id = seed_installation_token();
...
async fn create_pr_comment_success() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate::new(201).set_body_json(serde_json::json!({
            "id": 123456,
            "body": "This PR was closed because the task was abandoned.",
            "html_url": "https://github.com/djinnos/server/pull/42#issuecomment-123456"
        })))
        .mount(&server)
        .await;
    ...
    let comment = client
        .create_pr_comment(
            ...
            "server",
            42,
            "This PR was closed because the task was abandoned.",
        )
        .await
        .expect("create_pr_comment should succeed");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn create_pr_comment_failure_surfaces_error() {
    let server = MockServer::start().await;
    let install_id = seed_installation_token();

    Mock::given(method("POST"))
        .and(path("/repos/djinnos/server/issues/99/comments"))
        .respond_with(ResponseTemplate::new(403).set_body_json(serde_json::json!({
            "message": "Resource not accessible by integration"
        })))
        .mount(&server)
    ...
    let client = GitHubApiClient::for_installation_with_base_url(install_id, server.uri());
    let err = client
        .create_pr_comment("djinnos", "server", 99, "should fail")
        .await
