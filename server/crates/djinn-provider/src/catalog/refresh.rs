...
//!
//! This module lives in `djinn-provider` (next to [`CatalogService`]) so that
//! integration tests in downstream crates — notably `djinn-control-plane` — can
//! drive the **same** single-owner refresh loop against a mocked upstream URL,
//! proving that a changed models.dev response reaches the catalog through the
//! periodic tick rather than a test-only shortcut.

use std::time::{Duration, UNIX_EPOCH};

use crate::catalog::{CatalogService, RefreshStatus};
...
pub const MIN_PROVIDER_CATALOG_REFRESH_INTERVAL_SECS: u64 = 60;
pub const MAX_PROVIDER_CATALOG_REFRESH_INTERVAL_SECS: u64 = 86_400;
...
pub fn parse_refresh_interval_secs(value: Option<&str>) -> Result<u64, &'static str> {
...
/// Read and parse the refresh interval from the process environment, falling
/// back to the default on any error. Used by the server state constructor so the
/// loop and health endpoint share one cadence value.
...
pub fn boot_retry_backoff(failures: u32) -> Duration {
...
pub fn startup_jitter_max(interval: Duration) -> Duration {
