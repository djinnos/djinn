...
mod tests {
    ...
    // ───────────────────────────────────────────────────────────────────
    // Rust/SQL PREDICATE DRIFT GUARD
    // ───────────────────────────────────────────────────────────────────
    // The historical reclassification migration `84_reclassify_subscription_
    // session_cost_basis.sql` rewrites `sessions.cost_basis` from 'actual' to
    // 'projected' using SQL predicates that transcribe the *Rust* subscription
    // rules in this module (classify_provider / governable_subscription_for_
    // model / is_subscription_id). SQL cannot import the Rust functions, so the
    // two copies can silently drift as the builtin list evolves.
    //
    ...
    // representative provider/model ids. If a new builtin subscription provider
    // is added to `BUILTIN_PROVIDERS` but the SQL migration list is not updated,
