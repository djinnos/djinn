//! Retry utility for task transitions on locked database errors.

pub fn is_database_locked(error: &djinn_db::Error) -> bool {
    match error {
        djinn_db::Error::Sqlx(sqlx_err) => sqlx_err
            .as_database_error()
            .and_then(|db_err| db_err.code())
            .map(|code| matches!(code.as_ref(), "5" | "6" | "517" | "40001" | "40P01"))
            .unwrap_or_else(|| {
...
pub async fn retry_task_transition_on_locked<F, Fut, T>(mut op: F) -> Result<T, djinn_db::Error>
where
    F: FnMut() -> Fut,
