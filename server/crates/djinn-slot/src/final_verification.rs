...
use djinn_core::canonical_verify::{
    ...
    ResolvedVerificationSelectionV1, evaluate_freshness,
};
...
// recording logic. The additions below are: a per-session pre-lease gate
// (rate limiting), typed per-check results, and a typed tool outcome.
// ---------------------------------------------------------------------------
...
struct AgentRunCapture {
    ...
    ineligibility: Option<FinalVerificationIneligibilityReason>,
}
...
fn selection_surface(selection: &ResolvedVerificationSelectionV1) -> &'static str {
    match selection {
...
fn check_from_command(command: &FinalVerificationCommandEvidence) -> AgentVerificationCheckResult {
...
async fn coordinate_final_verification_core(
    request: FinalVerificationCoordinatorRequest,
    ctx: &SlotContext,
