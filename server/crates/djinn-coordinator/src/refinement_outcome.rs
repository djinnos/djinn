...
fn collect_adversary_round_objections(
...
impl CoordinatorActor {
    ...
    async fn process_adversary_outcome(&mut self, proposal_id: &str, state: &RefinementLoopState) {
        ...
        let (round_objections, carried_over) =
            collect_adversary_round_objections(&entries, round, run_start.as_deref());
...
mod tests {
    ...
    // ---- carry-over of prior-run objections into round 1 (starvation fix) ----
    ...
    fn round1_empty_current_run_carries_prior_run_objections() {
        ...
        let (objections, carried_over) =
            collect_adversary_round_objections(&entries, 1, run_start);
    ...
    fn round1_resolved_or_nonblocking_prior_run_stays_dry() {
        let entries = vec![
            ...
            objection_entry(1, true, Some("2026-07-09T10:30:00.000Z"), "2026-07-09T10:00:00.000Z"),
    ...
    fn round2_empty_current_run_does_not_carry_over() {
        ...
        let (objections, carried_over) =
            collect_adversary_round_objections(&entries, 2, run_start);
    ...
    fn round1_no_boundary_counts_all_as_current_run() {
        ...
        assert!(!carried_over, "no boundary → everything is in-run, no carry-over");
        assert_eq!(objections.len(), 2, "both round-1 entries count as current-run");
    }
}
