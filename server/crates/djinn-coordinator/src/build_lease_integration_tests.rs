...
// ─────────────────────────────────────────────────────────────────────────────
// Cap arming: the durable lease table is seeded to 0 by migration 139 and is
// written only by `set_cap`/`grant_next`. A deployment that never ran the
// operator arming sequence therefore keeps a durable cap of 0, and `grant_next`
// short-circuits on `occupied >= cap` before it ever reads the queue — every
// consumer denied for the life of the process while occupancy reads 0.
//
...
// rows, and zero warm Jobs created.

/// The exact production durable state: a database whose `build_lease_caps` row
/// is the migration-seeded 0 and whose `admission_handoff` table is empty.
