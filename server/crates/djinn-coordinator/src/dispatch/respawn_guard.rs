...
//! side-effects.  When a non-terminal attempt (`pending` or `submitted`) already
//! exists for the task+role, the guard defers dispatch and records a guard-only
//! `task_attempts` audit row via [`insert_guard_deferred`].  No dispatch,
//! provider, or reopen counters are incremented for guard deferrals.
//!
...
use djinn_db::{GuardDeferTaskAttemptParams, TaskAttemptRepository};
...
///
/// Consults [`TaskAttemptRepository::latest_pending_or_submitted`] for the
/// task/role pair.  If a non-terminal attempt already exists the dispatch is
/// deferred with [`GuardReason::RespawnGuard`].  Returns
/// [`RespawnGuardDecision::Allow`] otherwise.
///
...
mod tests {
    ...
    async fn guard_allows_when_no_prior_attempts() {
        ...
        let decision = run_respawn_guard(&db, &task.id, "worker").await;
