...
/// Regression for task pl4n (2026-07-23): a dispatch group carries both the
/// coordinator's `<task>:worker:<uuid>` dispatch-start row and the
/// supervisor's exact `task-run:<id>` row. `submit_work` advanced only the
/// newest pending row to `submitted`; the sibling `task-run:`-keyed row was
/// never terminalized on run completion, so after the PR reopened for rework
/// the guard saw a phantom `pending` attempt and deferred every dispatch for
/// ~45 minutes until the periodic reaper mislabeled the run `crashed`. The
/// supervisor's run-completion path now terminalizes the group's leftover
/// `pending` rows as `completed`; this test pins the guard-side contract: a
/// completed run leaves no pending `task-run:`-keyed attempt, and the guard
/// Allows the rework dispatch after the reopen.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
