...
use djinn_provider::github_api::{
    ...
    RequiredCheckReproduction, RequiredCheckUnreproducible, RequiredCheckUnreproducibleReason,
};
...
async fn run_required_ci_local_gate_before_pr_open(
    task: &djinn_core::models::Task,
    task_repo: &TaskRepository,
    ...
    let workspace = match mirror.clone_ephemeral(project_id, task_branch).await {
        ...
        Err(e) => {
            let results: Vec<LocalGateResult> = required_check_names
                .iter()
                .map(|check_name| {
                    LocalGateResult::Unreproducible(crate::local_gates::LocalGateUnreproducible {
                        required_check_name: check_name.clone(),
                        observed_head_sha: observed_head_sha.to_owned(),
                        reason: LocalGateUnreproducibleReason::CommandSpawnFailed,
                        details: Some(format!("could not materialize task worktree: {e}")),
                    })
                })
                .collect();
    ...
    for result in results {
            ...
            .await
        {
            tracing::warn!(
                task_id = %task.short_id,
                error = %e,
                "supervisor PR-open: failed to persist local CI gate result"
            );
        }
    }
}
...
async fn route_local_gate_block(
        ...
        .await
    {
        tracing::warn!(
            task_id = %task.short_id,
            error = %e,
            "supervisor PR-open: local CI gate remediation park transition skipped"
        );
    }
            ...
            .await
    {
        tracing::warn!(
            task_id = %task.short_id,
            error = %e,
            "supervisor PR-open: local CI gate unreproducible lead escalation skipped"
        );
    }
    let comment_payload = serde_json::json!({
        "body": format!(
