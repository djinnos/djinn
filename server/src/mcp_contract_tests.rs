...
// ── project_add repo-map refresh contract ────────────────────────────────────

#[cfg(test)]
mod project_add_repo_map_refresh {
    use serde_json::json;
    use tokio_util::sync::CancellationToken;

    use crate::test_helpers::{create_test_db, initialize_mcp_session, mcp_call_tool};
    use crate::watchers::REFRESH_SCHEDULED_TX;
    use djinn_provider::oauth::github_app::GITHUB_APP_OAUTH_DB_KEY;
    use djinn_provider::repos::CredentialRepository;

    /// ADR-043 contract proof: calling the real MCP `project_add` tool triggers
    /// initial repo-map refresh scheduling through the event-bus / watcher
    /// coordination path.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn project_add_schedules_repo_map_refresh() {
        // 1. Install test-only refresh observation channel.
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<String>();
        {
            let mut guard = REFRESH_SCHEDULED_TX.lock().unwrap();
            *guard = Some(tx);
        }

        // 2. Build AppState with a live broadcast channel (not noop).
        let db = create_test_db();
        let cancel = CancellationToken::new();
        let state = crate::server::AppState::new(db.clone(), cancel.clone());

        // 3. Spawn repo-map refresh watchers on the same event bus so they
        //    receive `project.created` events emitted by the MCP tool.
        ...
        // 5. Prepare a temp directory that looks like a git repo with a
        //    GitHub remote (project_add validates this).
        ...
        // 6. Build the router from the live AppState and call project_add
        //    through the full MCP HTTP stack.
        let app = crate::server::router(state);
        let session_id = initialize_mcp_session(&app).await;
        ...
        assert_eq!(
            ...
            "ok",
            "project_add should succeed; got: {result}"
        );
        ...
        assert_eq!(
            ...
            result["project"]["id"].as_str().unwrap(),
            "repo-map refresh should be scheduled for the newly added project"
        );

        // Cleanup: cancel watchers and clear the test hook.
        cancel.cancel();
        {
            let mut guard = REFRESH_SCHEDULED_TX.lock().unwrap();
            *guard = None;
