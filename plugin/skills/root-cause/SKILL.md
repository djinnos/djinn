---
name: root-cause
description: Analyze business problems, user needs, and strategic questions to find fundamental causes. Use when user asks "why do users churn", "what's the real problem", "why does this process fail", "root cause", "first principles", "jobs to be done", "JTBD", "five whys", "fundamental reason", "what do users actually need", "why isn't this working", or needs to understand underlying business/product causes rather than symptoms. NOT for technical debugging - use debugging skill instead.
allowed-tools: Read
---

# Root Cause - Business Problem Analysis

Dig beneath symptoms to find fundamental causes in business, product, and strategy contexts. Stop treating symptoms and solve actual problems.
...
Read [cookbook/jobs-to-be-done.md](./cookbook/jobs-to-be-done.md)
...
## When NOT to Use (Use Debugging Instead)

For **technical issues** - test failures, bugs, unexpected code behavior - use [[debugging]] skill instead. That skill has systematic phases for technical investigation.

This skill is for **business/product/strategy** root cause analysis:
- Why do users churn?
- Why does this process keep failing organizationally?
- What's the real problem we're solving?
- Why isn't this feature getting adoption?
