...
export interface FlameWrapElements {
  /** Kept for API compatibility; unused now that capture is gone. */
  source: HTMLCanvasElement;
  /** The element the flames are measured against. */
  content: HTMLElement;
...
export function createFlameWrap(
  ...
  /* The html-in-canvas capture path is deliberately gone: it needs a Chrome
     origin trial, and the flames read fine without it. The shader's
     uHasContent=0 branch draws fire around the element instead of through a
     captured bitmap, so `content` is only ever measured, never painted. */
  const htmlInCanvas = false;
  const contentDirty = false;
