...
test('repository-wired guard rejects staged scaffolding and generated schema surfaces', () => {
  ...
  try {
    ...
    mkdirSync(join(dir, 'server', 'crates', 'example', 'src'), { recursive: true });
    writeFileSync(join(dir, 'server', 'crates', 'example', 'src', 'test-runtime.rs'), 'std::fs::write(".djinn/settings.json", b"x");\n');
    execFileSync('git', ['-C', dir, 'add', 'server/crates/example/src/test-runtime.rs']);
    assert.equal(runGuard().status, 1, 'a production test-runtime.rs staged blob fails the actual guard');
    execFileSync('git', ['-C', dir, 'rm', '--cached', '-q', 'server/crates/example/src/test-runtime.rs']);
