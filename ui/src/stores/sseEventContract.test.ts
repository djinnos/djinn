...
describe("sseEventContract", () => {
  ...
  it("treats lagged and ping as explicit non-dispatch decisions", () => {
    expect(resolveServerSSEEventName("lagged")).toEqual({
      kind: "hydrate",
      reason: "lagged",
    });
    expect(resolveServerSSEEventName("ping")).toEqual({
      kind: "liveness",
      reason: "ping",
    });
  });
  ...
  it("keeps legacy or worker-consumed envelope names explicit instead of dispatching them", () => {
    ...
    expect(
      resolveServerSSEEventName("note.contradiction_candidates"),
    ).toMatchObject({
      kind: "ignore",
    });
  });
  ...
  it("warns at most once per unknown raw event name", () => {
    ...
    expect(resolveServerSSEEventName("task.archived")).toEqual({
      kind: "unknown",
    });
    expect(resolveServerSSEEventName("task.archived")).toEqual({
      kind: "unknown",
    });
    expect(resolveServerSSEEventName("proposal.merged")).toEqual({
      kind: "unknown",
    });
    ...
    expect(warn).toHaveBeenNthCalledWith(
      1,
      "[SSE] Unknown server event name: task.archived",
    );
    expect(warn).toHaveBeenNthCalledWith(
      2,
      "[SSE] Unknown server event name: proposal.merged",
    );
  });
});
