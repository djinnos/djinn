...
export function ProposalHistory({ detail }: { detail: ProposalDetail }) {
  ...
  // Each revision diffs against the one immediately before it (seq − 1).
  const bySeq = useMemo(() => {
    const m = new Map<number, (typeof revisions)[number]>();
    detail.revisions.forEach((r) => m.set(r.seq, r));
    return m;
  }, [detail.revisions]);
  ...
  const toggle = (seq: number) =>
    setOpen((cur) =>
      cur.includes(seq) ? cur.filter((s) => s !== seq) : [...cur, seq]
    );

  // A lone seed revision has nothing to compare against yet.
    ...
    <div className="space-y-2">
      ...
      <ul className="divide-y rounded-md border">
        {revisions.map((r) => {
          ...
          const isOpen = open.includes(r.seq);
